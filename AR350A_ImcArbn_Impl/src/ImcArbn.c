 ...
 *          File:  ImcArbn.c
 *     SW-C Type:  ImcArbn
 *  Generated at:  Wed Jan 18 16:58:21 2017
 *
...
static FUNC(void, IMCARBN_APPL_CODE) ImcArbnRx( P2VAR(Ary2D_u8_2_2, AUTOMATIC, RTE_IMCARBN_APPL_VAR) FrmFltCntr_Ary2D_u8,
...
static FUNC(ImcArbnRxRollgCntrSts1, IMCARBN_APPL_CODE) RxFrmVldChk( P2VAR(uint8, AUTOMATIC, RTE_IMCARBN_APPL_VAR) DataBuf_Cnt_T_u08,
                                                                    P2VAR(Ary1D_u8_2, AUTOMATIC, RTE_IMCARBN_APPL_VAR) FrmFltCntr_Cnt_T_u08,
...
static FUNC(uint32, IMCARBN_APPL_CODE)  GetBitMask(VAR(uint8, AUTOMATIC) NrOfBits_Cnt_T_u08);
 ...
 *********************************************************************************************************************/
        ...
        (Buf_Arg != NULL_PTR))
    {
        ...
        for(SigGroup_Cnt_T_u08 = RATEGROUPOFFS_CNT_U08[RateGroup_Arg] ;  SigGroup_Cnt_T_u08 < ( RATEGROUPOFFS_CNT_U08[RateGroup_Arg] + NRSIGGROUPINRATEGROUP_CNT_U08[RateGroup_Arg]); SigGroup_Cnt_T_u08++)
        {
            ...
            {
                Buf_Arg[FrmIdx_Cnt_T_u08] = PimTxBuf_u08[SigGroup_Cnt_T_u08][FrmIdx_Cnt_T_u08];
            }                        
        }        
...
{     
    ...
    for(SigGroup_Cnt_T_u08 = RATEGROUPOFFS_CNT_U08[RateGroup_Cnt_T_u08]; SigGroup_Cnt_T_u08 < (RATEGROUPOFFS_CNT_U08[RateGroup_Cnt_T_u08]+NRSIGGROUPINRATEGROUP_CNT_U08[RateGroup_Cnt_T_u08]); SigGroup_Cnt_T_u08++)
    { 
        /* Assume Signal Group is invalid */
        FrmSts_Cnt_T_enum = ROLLGCNTRINVLD;
...
static FUNC(uint32, IMCARBN_APPL_CODE) CreatSigGroupData(SigGroupRec1 const* SigGroup_T_rec) 
{
    ...
    for(SigIdx_Cnt_T_u16 = 0U; SigIdx_Cnt_T_u16 < SigGroup_T_rec->NrOfSig_u08; SigIdx_Cnt_T_u16++)
    {
        ...
        RetData_Uls_T_u32 |= ((SigVal_Uls_T_u32 & (GetBitMask((SigGroup_T_rec->SigPrm[SigIdx_Cnt_T_u16].DestSize_u08)))) << (SigGroup_T_rec->SigPrm[SigIdx_Cnt_T_u16].StrtLocn_u08));
    }
                                                        ...
                                                        VAR(ImcArbnRxDataSrc1, AUTOMATIC) RxDataSrc_Cnt_T_enum)
{
    ...
    for(SigIdx_Cnt_T_u16 = 0U; SigIdx_Cnt_T_u16 < SigGroup_T_rec->NrOfSig_u08; SigIdx_Cnt_T_u16++)
    {
        ...
        RxSigData_Uls_T_u32 = ((SigGroupRxData_Uls_T_u32) & ((GetBitMask((SigGroup_T_rec->SigPrm[SigIdx_Cnt_T_u16].DestSize_u08))) << (SigGroup_T_rec->SigPrm[SigIdx_Cnt_T_u16].StrtLocn_u08)));
        /* Shift to the right */
...
/*****************************************************************************************************************
  * Name        :   GetBitMask
  * Description :   Provides bit mask for given number of bits
  * Inputs      :   NrOfBits_Cnt_T_u08
  * Outputs     :   BitMask_Cnt_T_u32
  * Usage Notes :   None
******************************************************************************************************************/
static FUNC(uint32, IMCARBN_APPL_CODE)  GetBitMask(VAR(uint8, AUTOMATIC) NrOfBits_Cnt_T_u08)
{

    VAR(uint32, AUTOMATIC) BitMask_Cnt_T_u32;
    /* NrOfBits_Cnt_T_u08 range shall be 1 to 32 */    
    BitMask_Cnt_T_u32 = (uint32)(((uint32)1U << NrOfBits_Cnt_T_u08) - (uint32)1U);
    
    return (BitMask_Cnt_T_u32);
...
static FUNC(ImcArbnRxSts1, IMCARBN_APPL_CODE)  GetImcSigSts(VAR(ImcArbnRxExtdSts1, AUTOMATIC) SigExtdSts_Cnt_T_enum)
{
    ImcArbnRxSts1 RetSts_Cnt_T_enum;
