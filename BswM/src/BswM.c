...
# if ( BSWM_DEV_ERROR_DETECT == STD_OFF )
      /* #30 Verify that BswM is initialized - (This prevents queuing of new mode request if BswM_DeInit was called). */
      && BswM_IsInitialized()
# endif
      )
  {
...
#  if (BSWM_ABORTOFACTIONITEMS == STD_ON || BSWM_REPORTOFACTIONITEMS == STD_ON)
    if(BswM_GetActions(BswM_GetActionsIdxOfActionItems(itemId))(BswM_GetParameterIdxOfActionItems(itemId)) != E_OK) /* SBSW_BSWM_ACTIONFCTPTR */
    {
...
#  else
    (void)BswM_GetActions(BswM_GetActionsIdxOfActionItems(itemId))(BswM_GetParameterIdxOfActionItems(itemId)); /* SBSW_BSWM_ACTIONFCTPTR */
#  endif
  }
...
# endif
  ...
  /* ----- Development Error Checks ------------------------------------- */
# if (BSWM_DEV_ERROR_DETECT == STD_ON)
  /* #20 Check if BswM is initialized and passed mode is in range. */
  if (!BswM_IsInitialized())
  {
    errorId = BSWM_E_NO_INIT;
  }
...
# if(BSWM_DEFERREDRULES == STD_ON)
    /* #34 Arbitrate all rules which have at least one deferred port. */
