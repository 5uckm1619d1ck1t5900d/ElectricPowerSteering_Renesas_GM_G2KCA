...
| 2015-05-28   2.07.00   Mhe     Added:    ESCAN00083192: ggda.c includes "Desc.h" instead of "desc.h"
***************************************************************************************************************/
/*****************************
 include section 
*****************************/
...
/*    Version checks   */
/*---------------------*/
#if (DIAG_CANDESCGGDAEXT_GM_VERSION != 0x0207)
# error "Inconsistent header and source file version!"
#endif

#if (DIAG_CANDESCGGDAEXT_GM_RELEASE_VERSION != 0x00)
...
/*     Utilities       */
/*---------------------*/
#if defined (V_CONFIG_USE_DUMMY_STATEMENT) || defined (V_ENABLE_USE_DUMMY_STATEMENT)
...
/* Internal Functions  */
/*---------------------*/
#if defined (VGEN_CONFIG_VSTDLIB) || defined (V_CONFIG_VSTDLIB) || defined (VGEN_ENABLE_VSTDLIB)
...
static void GgdaProcessDisableNormalComm(GgdaContextIndexType context)
{
  if(ggdaContexts[context].dataLength == 0)
  {
#if (GGDA_CONFIG_SERVICE_28_PREHANDLER == GGDA_CONFIG_ON)
    vuint8 applResult;
    applResult = ApplGgdaMayDisableNormalComm(context);
    if (applResult != kDescOk)
    { 
    ...
    {
      Nm_Status nmResult;
      nmResult = IlNwmNormalCommHalted(ggdaCanConfig[context].canChannel);
      if (nmResult != NM_OK)
      {
        ggdaContexts[context].errorRegister = kGgdaNrcConditionsNotCorrect;
      }
