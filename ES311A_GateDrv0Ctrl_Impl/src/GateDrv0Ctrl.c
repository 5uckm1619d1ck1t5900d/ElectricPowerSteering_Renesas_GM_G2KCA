...
#define  GATEDRV0MASK0VAL_CNT_U16        	(26112U)   
...
static FUNC(boolean, GateDrv0Ctrl_CODE) GateDrvDetermineGenericFlt( void );
...
static FUNC(void, GateDrv0Ctrl_CODE) OffStVrfySt( void )
{
	...
	{	/* OffSt Verification Chk and Transition to Config State */
		...
		{	/* OffStChk Incomplete */
			...
			if( (Flt_Cnt_T_logl == FALSE) && (*Rte_Pim_GateDrv0OffStChkIdx() == ELECGLBPRM_GATEDRVOFFSTCHKSIZE_CNT_U08) )
...
static FUNC(void, GateDrv0Ctrl_CODE) OperFltMonrSt( void )
{
	...
	VAR(boolean,  AUTOMATIC)  GenGateDrvFlt3_Cnt_T_logl;
	...
	/***********************************************************************************/
	if( ((*Rte_Pim_GateDrv0StsVal()) & 0x4000U) != 0U )
	{	/* Status Register indicates Fault */
		...
		GenGateDrvFlt3_Cnt_T_logl = GateDrvDetermineGenericFlt(); 
...
static FUNC(boolean, GateDrv0Ctrl_CODE) GateDrvDetermineGenericFlt( void )
{
	...
	/*** --------------------------------------------------------------------------- ***/
	if( ((*Rte_Pim_GateDrv0StsVal()) & 0x2800U) != 0U )
	{
		GenGateDrvFlt_Cnt_T_logl = TRUE;
	...
	/*** --------------------------------------------------------------------------- ***/
	if( ((*Rte_Pim_GateDrv0StsVal()) & 0x1000U) != 0U )
	{
		GenGateDrvFlt_Cnt_T_logl = TRUE;
	...
	/*** --------------------------------------------------------------------------- ***/
	if( ((*Rte_Pim_GateDrv0StsVal()) & 0x0400U) != 0U )  
	{
		GenGateDrvFlt_Cnt_T_logl = TRUE;
