...
#if (  (ECUM_SW_MAJOR_VERSION != (0x05u)) \
...
/* vendor specific version information is BCD coded */
#if (  (ECUM_CBK_SW_MAJOR_VERSION != (0x05u)) \
...
# if(ECUM_SUPPORT_COMM == STD_ON) /* COV_ECUM_06 */
/**********************************************************************************************************************
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalBswMNotifications(void)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* Notify new wakeup events to the BswM */
  if (bufferedWupSource != 0u)
  {
...
# endif
    ...
    /* Remove the here notified sources but do not touch potential new buffered  */
    EcuM_EnterCriticalArea_0();
    EcuM_BswM_BufferedWakeups &= (~bufferedWupSource);
    EcuM_LeaveCriticalArea_0();
  }
}
#endif
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalDeterminePbConfiguration(void)
{
  ...
  uint8 ecumErrorId = ECUM_E_NO_ERROR;
...
# if (ECUM_USE_ECUM_BSW_ERROR_HOOK == STD_ON)
  if(ecumErrorId != ECUM_E_NO_ERROR)
  {
...
ECUM_LOCAL boolean EcuM_WaitOnNvMWriteAll(void)
{
  ...
  else if ( EcuM_GetNvMWriteAllStatus() == ECUM_NVM_WRITEALL_STARTED)
  {
...
ECUM_LOCAL FUNC(boolean, ECUM_CODE) EcuM_QryComMRunRequest(void)
{
  ...
  for (loopCount = 0u; loopCount < EcuM_GetSizeOfComMChannels(); loopCount++)
  {
    ...
    if (E_OK == ComM_GetState((NetworkHandleType) EcuM_GetComMChannels(loopCount), &currentState))
    {
      /* If at least one channel is in COMM_NO_COM_NO_PENDING_REQUEST the function returns and EcuM has to stay in Run State. */
...
# if ( (ECUM_INCLUDE_NVRAM_MGR == STD_ON) && (ECUM_TIMEOUTOFWAKEUPSOURCELIST == STD_ON))
    else
    {
      (void) EcuM_WaitOnNvMCancelWriteAll();
    }
...
# endif
    {
      ...
      {
# if (ECUM_SLEEPMODELIST == STD_ON)
#  if (ECUM_TIMEOUTOFWAKEUPSOURCELIST == STD_ON) /* COV_ECUM_06 */
...
# if (ECUM_SUPPORT_DEM == STD_ON)
        Dem_Init( ECUM_DEM_CONFIG_POINTER);
...
# endif
      {
# if(ECUM_SUPPORT_RTE == STD_ON) /* COV_ECUM_06 */
        (void) Rte_Stop();
# endif
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalKillAllRequests(uint8 mask)
{
  ...
  {
    EcuM_SetUserTable(user, (EcuM_UserType)(EcuM_GetUserTable(user) & (EcuM_UserType)(~mask)));
  }
  ...
  if (mask == ECUM_USER_REQUEST_RUN_MASK)
  {
    ...
    {
      /* #35 Stop the EcuM self run request timeout. */
...
# endif
      ...
      if ((EcuM_IntendedRestartID_One != ECUM_MAGIC_ONE) || (EcuM_IntendedRestartID_Two != ECUM_MAGIC_TWO))
      {
        EcuM_Clock = 0u;
      }
...
FUNC(void, ECUM_CODE) EcuM_SetWakeupEvent(VAR(EcuM_WakeupSourceType, AUTOMATIC) WakeupSource)
{
  ...
  else
  {
    ...
    {
# if(ECUM_CHECKWAKEUPTIMEOFWAKEUPSOURCELIST == STD_ON)
      /* #32 Remove the current source from the pending check wakeup bit mask. */
      EcuM_PendingCheckWakeups &= (EcuM_WakeupSourceType) (~WakeupSource);
# endif
...
FUNC(Std_ReturnType, ECUM_CODE) EcuM_AbortWakeupAlarm(VAR(EcuM_UserType, AUTOMATIC) user)
{
  ...
  else
  {
    ...
    if (EcuM_GetAlarmClockTimeOut(AlarmClockUser) != 0u)
    {
      ...
      EcuM_GetAlarmClockTimeOut(AlarmClockUser) = 0u; /* SBSW_ECUM_ACCESSALARMCLOCK */
 ...
 *
     MD_EcuM_0715:
     Reason:     Nesting of control structures is necessary for optimize code runtime.
     Risk:       Some compilers could have problems with this.
     Prevention: Verified during integration with target compiler.
     ...
     MD_EcuM_3684:
     Reason:     ECUM supports post build variant, therefore the size is not fix.
     Risk:       Array access out of bounds not found by compiler.
     Prevention: Covered by code review.
...
\ID COV_ECUM_06
  \ACCEPT  TX
  \REASON  EcuM Fix is currently not covered in all variants.
