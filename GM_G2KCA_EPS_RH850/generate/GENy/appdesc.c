/* -----------------------------------------------------------------------------
  ...
  Generated by CANdelaGen, Fri Jul 15 09:12:19 2016
...
{
	if (NOERR_CNT_U16 == EnvtlChk(TESTVEHSPD_CNT_U08, 3.0f))
	{
...
void DESC_API_CALLBACK_TYPE ApplDescReadFailureRecordData(DescMsgContext* pMsgContext)
{
	...
	if (pMsgContext->reqData[0U] == 0x01U)
	{
		...
		for (Idx_Cnt_T_u08 = 1U; Idx_Cnt_T_u08 <= Dem_Cfg_GlobalDtcCount(); Idx_Cnt_T_u08 += 1U)
		{
			/* Fetch DTC status */
			...
			(ElpdTi_mS_T_u32 > 10000U))
		{
			    ...
			    (DescGetStateSecurity_Access() != kDescStateSecurity_AccessUnlock_Device_Control))
			{
				...
				if ((CustMfgEnaSt_Cnt_T_enum == MFGENAST_MFGMOD) ||
					...
					(pMsgContext->resData[1U] != 0xFFU) ||
					(pMsgContext->resData[2U] != 0xFFU) ||
					(pMsgContext->resData[3U] != 0xFFU) ||
					(pMsgContext->resData[4U] != 0xFFU))
				{
					pMsgContext->resDataLen = 5U;
...
void DESC_API_CALLBACK_TYPE ApplDescRequestDownload(DescMsgContext* pMsgContext)
{
	...
	/* TODO: This is a short term fix, it doesn't store NvM before resetting the micro */
	NxtrSwRst(MCUDIAGC_FLSPROGMREQ, Length_Cnt_T_u32);
...
void DESC_API_CALLBACK_TYPE ApplDescPreDisableNormalCommunication(vuint8 iContext)
{
	...
	/* Engine must be off and vehicle speed must either be invalid or less than 3 kph */
	if ((EngRunActv_Cnt_T_logl == TRUE) || ((VehSpdVld_Cnt_T_logl == TRUE) && (VehSpd_Kph_T_f32 >= 3.0f)))
	{
		/* Failed rationality check -> NRC 0x22 */
...
void DESC_API_CALLBACK_TYPE ApplDescPreControlCPID_02_Steering_Assist_Control(vuint8 iContext)
{
	...
	RespCod_Cnt_T_u16 = EnvtlChk(TESTSECUACS_CNT_U08 | TESTVEHSPD_CNT_U08 | TESTENGRUN_CNT_U08, 10.0f);
...
void DESC_API_CALLBACK_TYPE ApplDescPreControlCPID_FD_System_Basic_Functions(vuint8 iContext)
{
	...
	else
	{
		...
		RespCod_Cnt_T_u16 = EnvtlChk(TESTSECUACS_CNT_U08 | TESTVEHSPD_CNT_U08, 10.0f);
	}
...
void ApplDescGetDtcStatusByMask(vuint16 iterPos, vuint8 statusMask)
{
	...
	for (Idx_Cnt_T_u08 = (uint8)(iterPos + 1U);
		((Idx_Cnt_T_u08 < Dem_Cfg_GlobalDtcCount()) && (FALSE == StsFound_Cnt_T_lgc));
		Idx_Cnt_T_u08 += 1U)
	{
		(void)Dem_GetEventStatus(Idx_Cnt_T_u08, &UdsSts_Cnt_T_u08);
	...
	{
		DescRdiDtcStatusByMaskNotFound(Dem_Cfg_GlobalStatusMask());
	}
}
