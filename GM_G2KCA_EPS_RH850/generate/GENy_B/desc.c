/* -----------------------------------------------------------------------------
  ...
  Description: Implementation of CANdesc.
                ...
                EcuDocFile:   C:\Workspace\_EA4\T1xx\GM_T1XX_EPS_RH850_Working\GM_T1XX_EPS_RH850\tools\GENy\EPS_T1XX_Nexteer_19.0.0_60.cdd
                Variant:      CommonDiagnostics

  Generated by CANdelaGen, Thu Feb 02 11:41:22 2017
...
#define DESC_IMPLEMENTATION_MAGIC_NUMBER                             16339
...
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
...
# define RESOURCE_PARAM_USAGE_ONLY(param)                            param
# define RESOURCE_PARAM_USAGE_FIRST(param)                           (param),
#else
...
# define RESOURCE_PARAM_USAGE_ONLY(iter)
# define RESOURCE_PARAM_USAGE_FIRST(param)
#endif
...
#if (kDescUudtNetNumChannels > 1)
# define CHANNEL_ITER_TYPE_ONLY                                      t_descUudtNetChannelIter
# define CHANNEL_ITER_TYPE_FIRST                                     t_descUudtNetChannelIter
# define CHANNEL_ITER_VALUE                                          channelIter
# define CHANNEL_ITER_ONLY                                           CHANNEL_ITER_VALUE
# define CHANNEL_ITER_FIRST                                          CHANNEL_ITER_VALUE,
# define CHANNEL_ITER_DEF_LOCAL                                      CHANNEL_ITER_TYPE_ONLY CHANNEL_ITER_ONLY;
# define CHANNEL_ITER_VALUE_INCREMENT                                CHANNEL_ITER_VALUE++
# define CHANNEL_ITER_USAGE(iter)                                    iter
#else
# define CHANNEL_ITER_TYPE_ONLY                                      void
# define CHANNEL_ITER_TYPE_FIRST
# define CHANNEL_ITER_VALUE                                          0
# define CHANNEL_ITER_ONLY
# define CHANNEL_ITER_FIRST
# define CHANNEL_ITER_DEF_LOCAL
# define CHANNEL_ITER_VALUE_DECREMENT
# define CHANNEL_ITER_USAGE(iter)
#endif
...
# define CAN_MSG_ITER_USAGE_FIRST(iter)                              (iter),
#else
...
# define CAN_MSG_ITER_USAGE_FIRST(iter)
#endif
...
# define CAN_MSG_PARAM_USAGE(param)                                  param
#else
...
# define CAN_MSG_PARAM_USAGE(param)
#endif
...
/* Abstraction for Ecu and Channel info table */
#if (kDescUudtNetNumEcus > 1)
# define DESC_ECU_INFO_ECU_ID                                        (g_descUudtNetEcuInfoTable[comMultipleECUCurrent].ecuId)
# define DESC_ECU_INFO_CHNL_BASE_IDX                                 (g_descUudtNetEcuInfoTable[comMultipleECUCurrent].channelInfoBaseIdx)
# define DESC_ECU_INFO_CHNL_BASE_IDX_NXT                             (g_descUudtNetEcuInfoTable[comMultipleECUCurrent + 1].channelInfoBaseIdx)
# define DESC_ECU_INFO_NUM_CHANNELS                                  (DESC_ECU_INFO_CHNL_BASE_IDX_NXT - DESC_ECU_INFO_CHNL_BASE_IDX)
#else
# define DESC_ECU_INFO_ECU_ID                                        0
# define DESC_ECU_INFO_CHNL_BASE_IDX                                 0
# define DESC_ECU_INFO_CHNL_BASE_IDX_NXT                             kDescUudtNetNumChannels
# define DESC_ECU_INFO_NUM_CHANNELS                                  kDescUudtNetNumChannels
#endif

#if (kDescUudtNetNumChannels > 1)
# define DESC_CHNL_INFO_CHNL_ID(idx)                                 (g_descUudtNetChannelInfoTable[(idx)].channelId)
# define DESC_CHNL_INFO_MSG_BASE_IDX(idx)                            (g_descUudtNetChannelInfoTable[(idx)].msgInfoBaseIdx)
# define DESC_CHNL_INFO_MSG_BASE_IDX_NXT(idx)                        (g_descUudtNetChannelInfoTable[(idx) + 1].msgInfoBaseIdx)
#else
# define DESC_CHNL_INFO_CHNL_ID(idx)                                 0
# define DESC_CHNL_INFO_MSG_BASE_IDX(idx)                            0
# define DESC_CHNL_INFO_MSG_BASE_IDX_NXT(idx)                        kDescUudtNetNumCanMsgs
#endif
...
#define kDescUudtNetChannelRefInvalid                                (t_descUudtNetChannelIter)0xff
...
#define kDescUudtNetCanResEngineSentOk                               (t_descUudtNetResourceState)0x08
#define kDescUudtNetCanResEngineDoSend                               (t_descUudtNetResourceState)0x10
#define kDescUudtNetCanResEngineSentNotOk                            (t_descUudtNetResourceState)0x20

#define kDescUudtNetCanEngineIdle                                    (t_descUudtNetCANMsgState)0x00
#define kDescUudtNetCanEngineLocked                                  (t_descUudtNetCANMsgState)0x01
#define kDescUudtNetCanEngineSentOk                                  (t_descUudtNetCANMsgState)0x02
#define kDescUudtNetCanEngineSentNotOk                               (t_descUudtNetCANMsgState)0x04
...
#if !defined kDescOemSpontanResComChannel
/* Default initialization */
# define kDescOemSpontanResComChannel                                0
#endif
...
/* SvcHead table size. */
#define kDescSvcHeadNumItems                                         18
/* SvcInst table size. */
#define kDescSvcInstNumItems                                         68
/* SvcInstHeadEx table size. */
#define kDescSvcInstHeadExtNumItems                                  58
/* Context modes */
...
#define kDescDefaultSessionSvcInstEntry                              33
...
#define kDescOemNrcTooManyPIDs                                       kDescNrcSubfunctionNotSupported
...
#define kDescDynDpidMaxNumElements                                   1
...
typedef vuint8  t_descUudtNetCANMsgState;
...
/* Index of the channel table */
typedef vuint8_least t_descUudtNetChannelIter;

/* CANmsg instanced state machine */
typedef struct
{
  t_descUudtNetConfTimer timer;
  t_descUudtNetCANMsgState engine;
...
typedef struct
{
  vuint8 baseIdx;
  vuint8 resCount;
#if defined (DESC_UUDTNET_ENABLE_ORDERED_TX)
  vuint8 freeCount;
#endif
  vuint8 size;
}t_descUudtNetResMgrCtxt;
...
{
  vuint8  channelId;
  vuint8  msgInfoBaseIdx;
}t_descUudtNetChannelInfoStruct;

typedef struct
{
  vuint8  ecuId;
  vuint8  channelInfoBaseIdx;
}t_descUudtNetEcuInfoStruct;
...
static void DescUudtNetCanTxConfirmation(RESOURCE_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status);
static void DescUudtNetCanTxMsgEnd(RESOURCE_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status);
static void DescUudtNetResourceMgrConfirmation(RESOURCE_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status);
static void DescUudtNetResourceMgrMsgEnd(RESOURCE_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status);
...
static t_descUudtNetInfoPool g_descUudtNetInfoPoolCAN[kDescUudtNetMaxNumCanMsgsPerChannel];

#if defined(DESC_UUDTNET_ENABLE_DATA_BUFFERS)
/* Optional data queue */
static vuint8 g_descUudtNetDataQueueBuffers[kDescUudtNetMaxNumCanMsgsPerChannel][kDescUudtNetFrameLen];
#endif

static t_descUudtNetResourceManager   g_descUudtNetResourceManager[kDescUudtNetMaxNumCanMsgsPerChannel];
static t_descUudtNetResMgrCtxt        g_descUudtNetResMgrCtxt;

/* CAN msg specific monitoring manager */
static t_descUudtNetCANTxManager  g_descUudtNetCanMsgManager[kDescUudtNetMaxNumCanMsgsPerChannel];
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
...
V_MEMROM0 static V_MEMROM1 t_descUudtNetMsgInfoStruct V_MEMROM2 g_descUudtNetCanMsgInfoTable[2] = 
{
  { 4, (TxDataPtr)UUDT_Resp_From_EHPS_EPS_HS._c }, 
  { 8, (TxDataPtr)UUDT_Resp_From_EPS_EPS_JR_CE._c }
};
/* Table of all configured CAN channels used for UUDT transmissions. */
V_MEMROM0 static V_MEMROM1 t_descUudtNetChannelInfoStruct V_MEMROM2 g_descUudtNetChannelInfoTable[3] = 
{
  {   0, 0 }, 
  {   1, 1 }, 
  { 255, 2 }
...
V_MEMROM0 static V_MEMROM1 DescMsgItem V_MEMROM2 g_descSidMap[kDescSvcIdMapSize] = 
{
  ...
  0xFF, 0xFF, 0xFF, 0xFF, 0x09, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x0A, 0xFF, 0xFF, 0x0B, 0xFF, 
  0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 
  0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 
  0xFF, 0xFF, 0x0C, 0xFF, 0xFF, 0x0D, 0xFF, 0xFF, 0xFF, 0x0E, 0x0F, 0xFF, 0xFF, 0xFF, 0x10
};
...
V_MEMROM0 static V_MEMROM1 DescSvcHead V_MEMROM2 g_descSvcHead[kDescSvcHeadNumItems] = 
{
  ...
  { 0x01, 0, 0, 0x03, 0x03, 0, 1, 0, 0x00, 33, 31 } /* $20 */, 
  { 0x01, 0, 0, 0x03, 0x03, 0, 1, 0, 0x00, 34, 31 } /* $22 */, 
  { 0x01, 0, 0, 0x01, 0x01, 0, 1, 0, 0x00, 35, 31 } /* $23 */, 
  { 0x01, 1, 1, 0x03, 0x03, 0, 1, 1, 0x00, 36, 31 } /* $27 */, 
  { 0x01, 0, 0, 0x03, 0x03, 0, 1, 0, 0x00, 40, 35 } /* $28 */, 

  { 0x01, 0, 0, 0x01, 0x01, 0, 1, 0, 0x00, 41, 35 } /* $2C */, 
  { 0x01, 0, 0, 0x01, 0x01, 0, 1, 0, 0x00, 42, 35 } /* $34 */, 
  { 0x03, 1, 1, 0x01, 0x01, 0, 1, 0, 0x00, 43, 35 } /* $3B */, 
  { 0x01, 0, 0, 0x03, 0x01, 0, 1, 0, 0x00, 52, 44 } /* $3E */, 
  { 0x01, 0, 0, 0x03, 0x03, 0, 1, 0, 0x00, 53, 44 } /* $A2 */, 
  { 0x01, 1, 0, 0x03, 0x03, 0, 0, 1, 0x00, 54, 44 } /* $A5 */, 
  { 0x01, 1, 0, 0x03, 0x03, 0, 0, 1, 0x00, 56, 46 } /* $A9 */, 
  { 0x01, 1, 0, 0x01, 0x01, 0, 0, 1, 0x00, 57, 47 } /* $AA */, 

  { 0x03, 1, 1, 0x01, 0x01, 0, 1, 0, 0x00, 62, 52 } /* $AE */, 
  { 0x00, 0, 0, 0x00, 0x00, 0, 1, 0, 0x00, 68, 58 } /* $FF */
};
...
{
  0x02, 0x90, 0x97, 0x98, 0x99, 0x9A, 0x9F, 0xA0, 0xAB, 0xB0, 0xB3, 0xB4, 0xC0, 0xC1, 0xC2, 0xC3 /* $1A */, 
  0xC4, 0xCB, 0xCC, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xDB, 0xDC, 0xDE, 0xF0, 0xF3, 0xF4, 0xF6 /* $1A */, 
...
#if (kDescUudtNetNumChannels > 1)
# define DescUudtMsgInfoAt(iter)                                     (g_descUudtNetCanMsgInfoTable[g_descUudtNetResMgrCtxt.baseIdx + (iter)])
#else
# define DescUudtMsgInfoAt(iter)                                     (g_descUudtNetCanMsgInfoTable[0 + (iter)])
#endif
...
#endif
#define DescUudtNetLL_InitCanMsgManager()                            /* Not used */
...
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
  RESOURCE_ITER_VALUE = kDescUudtNetMaxNumCanMsgsPerChannel;
...
DESCNET_UUDT_STATIC void DescUudtNetCANInit(void)
{
  ...
  CAN_MSG_ITER_VALUE = kDescUudtNetMaxNumCanMsgsPerChannel;
  ...
  DescUudtNetLL_InitCanMsgManager();

#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
  /* Resource manager is empty */
  RESOURCE_ITER_VALUE = kDescUudtNetMaxNumCanMsgsPerChannel;
  while(RESOURCE_ITER_VALUE != 0)
#endif
  {
  ...
  g_descUudtNetResMgrCtxt.baseIdx = kDescUudtNetNumCanMsgs;
  g_descUudtNetResMgrCtxt.resCount = 0;
#if defined (DESC_UUDTNET_ENABLE_ORDERED_TX)
  g_descUudtNetResMgrCtxt.freeCount = 0;
#endif
  g_descUudtNetResMgrCtxt.size = DESC_CHNL_INFO_MSG_BASE_IDX_NXT(0);
}
...
static void DescUudtNetInitCanMsgManager(CAN_MSG_FORMAL_PARAM_DEF_ONLY)
{
  ...
  g_descUudtNetCanMsgManager[CAN_MSG_PARAM_VALUE].engine     = kDescUudtNetCanEngineIdle;
}
...
#else
# if (kDescUudtNetNumChannels == 1) /* Context switch only necessary when multiple channels exist */
  g_descUudtNetInfoPoolCAN[RESOURCE_PARAM_VALUE].resDataPtr       = DescUudtNetMsgBuffer(RESOURCE_PARAM_VALUE);
# else
  /* Multi channel case: init is done when the context is switched */
# endif
#endif
#if defined(DESC_UUDTNET_ENABLE_MULTI_BUS_SUPPORT)
...
DESCNET_UUDT_STATIC void DescUudtNetCANTimerTask(void)
{
  ...
  {
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
    CAN_MSG_ITER_VALUE = g_descUudtNetResMgrCtxt.size;
    while(CAN_MSG_ITER_VALUE != 0)
#endif
    {
      CAN_MSG_ITER_VALUE_DECREMENT;
      ...
      if(g_descUudtNetCanMsgManager[CAN_MSG_ITER_VALUE].timer != 0)
      {
        ...
        if(g_descUudtNetCanMsgManager[CAN_MSG_ITER_VALUE].timer == 0)
        {
          ...
          if(g_descUudtNetResourceManager[DescUudtNetResHandleRef(CAN_MSG_ITER_VALUE)].engine != kDescUudtNetCanResEngineSentOk)
          {
            /* Message obsolete */
            DescUudtNetLL_CancelTransmit(DescUudtNetMsgHandle(CAN_MSG_ITER_VALUE));
            DescUudtNetLL_InterruptRestore();
            /* Message not sent - error */
            DescUudtNetCanTxConfirmation(CAN_MSG_ITER_USAGE_FIRST(CAN_MSG_ITER_VALUE)
...
DESCNET_UUDT_STATIC void DescUudtNetCANStateTask(void)
{
  ...
  RESOURCE_ITER_VALUE = g_descUudtNetResMgrCtxt.size;
  while(RESOURCE_ITER_VALUE != 0)
#endif
  {
    RESOURCE_ITER_VALUE_DECREMENT;

    /*CAN message manager steps */
    if(g_descUudtNetCanMsgManager[RESOURCE_ITER_VALUE].engine == kDescUudtNetCanEngineSentOk)
    {
      DescUudtNetCanTxMsgEnd(CAN_MSG_ITER_USAGE_FIRST(RESOURCE_ITER_VALUE) kDescUudtNetworkOk);
    }
    else if(g_descUudtNetCanMsgManager[RESOURCE_ITER_VALUE].engine == kDescUudtNetCanEngineSentNotOk)
    {
      DescUudtNetCanTxMsgEnd(CAN_MSG_ITER_USAGE_FIRST(RESOURCE_ITER_VALUE) kDescUudtNetworkFailed);
    }
    else
    {
      /* Idle or locked */
    ...
    /* Release slot if already sent */
    if(g_descUudtNetResourceManager[RESOURCE_ITER_VALUE].engine == kDescUudtNetCanResEngineSentOk)
    {
      DescUudtNetResourceMgrMsgEnd(RESOURCE_ITER_USAGE_FIRST(RESOURCE_ITER_VALUE) kDescUudtNetworkOk);
    }
    /* Release slots if transmission failed */
    else if(g_descUudtNetResourceManager[RESOURCE_ITER_VALUE].engine == kDescUudtNetCanResEngineSentNotOk)
    {
#if defined (DESC_UUDTNET_ENABLE_RETRANSMIT_ON_FAILED_TX)
      /* Set state to resend the message */
      g_descUudtNetResourceManager[RESOURCE_ITER_VALUE].engine = kDescUudtNetCanResEngineDoSend;
#else
      /* Unsuccesfull sending -> end it */
      DescUudtNetResourceMgrMsgEnd(RESOURCE_ITER_USAGE_FIRST(RESOURCE_ITER_VALUE) kDescUudtNetworkFailed);
#endif
    }
    else
    {
    ...
    if((g_descUudtNetResourceManager[RESOURCE_ITER_VALUE].engine & (kDescUudtNetCanResEngineDoSend | kDescUudtNetCanResEngineResend)) != 0)
    {
      /* Remove the retry state (made in the function below too. */
      /* g_descUudtNetResourceManager[RESOURCE_PARAM_VALUE].engine = kDescUudtNetCanResEngineLocked;*/
      DescUudtNetTryToSendMsg(RESOURCE_ITER_USAGE_ONLY(RESOURCE_ITER_VALUE));
    }
  }
}
...
{
  RESOURCE_ITER_DEF_LOCAL
  CHANNEL_ITER_DEF_LOCAL

  if(g_descUudtNetResMgrCtxt.resCount >= g_descUudtNetResMgrCtxt.size)
  {
    /* No free slots in the resource manager */
    return V_NULL;
  }

#if (kDescUudtNetNumChannels > 1) /* Context switch only necessary when multiple channels exist */
  /* Get the right channel info */
# if defined(DESC_UUDTNET_ENABLE_DYN_MULTICAN_SUPPORT)
  /* Do a lookup */
  CHANNEL_ITER_VALUE = DESC_ECU_INFO_CHNL_BASE_IDX;
  while(CHANNEL_ITER_VALUE != DESC_ECU_INFO_CHNL_BASE_IDX_NXT)
  {
    if(busInfo->busProperty.can.canChannel == DESC_CHNL_INFO_CHNL_ID(CHANNEL_ITER_VALUE))
    {
      break; /* Channel found in table, do not procede with search */
    }
    CHANNEL_ITER_VALUE_INCREMENT;
  }
#  if defined (DESC_ENABLE_DEBUG_INTERNAL)
  if(CHANNEL_ITER_VALUE == DESC_ECU_INFO_CHNL_BASE_IDX_NXT)
  {
    /* requested channel not found */
    DescUudtNetCanAssertUserAlways(kDescNetAssertUnknownUudtChannel);
    return V_NULL;
  }
#  endif
# else
  /* All ECUs have only one channel -> use direct access */
  CHANNEL_ITER_VALUE = DESC_ECU_INFO_CHNL_BASE_IDX;
# endif

  if(g_descUudtNetResMgrCtxt.baseIdx != DESC_CHNL_INFO_MSG_BASE_IDX(CHANNEL_ITER_VALUE)) /* Context switch required at all? */
  {
    if(0 == g_descUudtNetResMgrCtxt.resCount) /* Context switch allowed? (Any active transmissions?) */
    {
      /* No active transmissions -> do the context switch */
      g_descUudtNetResMgrCtxt.baseIdx = DESC_CHNL_INFO_MSG_BASE_IDX(CHANNEL_ITER_VALUE);
      g_descUudtNetResMgrCtxt.size = DESC_CHNL_INFO_MSG_BASE_IDX_NXT(CHANNEL_ITER_VALUE) - g_descUudtNetResMgrCtxt.baseIdx;

# if defined(DESC_UUDTNET_ENABLE_DATA_BUFFERS)
      /* Already initialized during init */
# else
      {
#  if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
        RESOURCE_ITER_VALUE = g_descUudtNetResMgrCtxt.size;
...
#  endif
        {
          RESOURCE_ITER_VALUE_DECREMENT;
          g_descUudtNetInfoPoolCAN[RESOURCE_ITER_VALUE].resDataPtr = DescUudtNetMsgBuffer(RESOURCE_ITER_VALUE);
        }
      }
# endif
    }
    ...
    {
      /* Active transmissions -> don't change context and dont reserve object */
      return V_NULL;
    }
  } /* else - same context required -> don't change context */
#endif
...
#endif
      return &g_descUudtNetInfoPoolCAN[RESOURCE_ITER_VALUE];
    }/* else - resource occupied -> try next one */
  }
  return V_NULL;
}
...
DESCNET_UUDT_STATIC void DescUudtNetCANTransmitResponse (t_descUudtNetInfoPoolPtr infoPool)
{
  ...
  g_descUudtNetResMgrCtxt.resCount++;
#endif
...
static void DescUudtNetTryToSendMsg (RESOURCE_FORMAL_PARAM_DEF_ONLY)
{
  ...
  if(g_descUudtNetResourceManager[RESOURCE_PARAM_VALUE].engine == kDescUudtNetCanResEngineDoSend)
  {
    ...
    g_descUudtNetCanMsgManager[CAN_MSG_ITER_RESOURCE_VALUE].engine    = kDescUudtNetCanEngineLocked;    /* Reserve the CAN message */
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
...
/*******************************************************************************
* NAME:              DescUudtNetResourceMgrConfirmation
*
* CALLED BY:         DescUudtNetCAN
* PRECONDITIONS:
*
...
static void DescUudtNetResourceMgrConfirmation(RESOURCE_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status)
{
  if(status == kDescUudtNetworkOk)
  {
    g_descUudtNetResourceManager[RESOURCE_PARAM_VALUE].engine = kDescUudtNetCanResEngineSentOk;
  }
  else /* status == kDescUudtNetworkFailed */
  {
    /* Signal unsuccessful sending */
    g_descUudtNetResourceManager[RESOURCE_PARAM_VALUE].engine = kDescUudtNetCanResEngineSentNotOk;
  }
}

/*******************************************************************************
* NAME:              DescUudtNetResourceMgrMsgEnd
*
* CALLED BY:         DescUudtNetCAN
* PRECONDITIONS:
*
...
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
# if defined (DESC_UUDTNET_ENABLE_ORDERED_TX)
  g_descUudtNetResMgrCtxt.freeCount++;
  if(g_descUudtNetResMgrCtxt.freeCount == g_descUudtNetResMgrCtxt.resCount)
  {
    /* Release free slots only if all occupied UUDTs are sent */
    g_descUudtNetResMgrCtxt.resCount = 0;
    g_descUudtNetResMgrCtxt.freeCount = 0;
  }
# else
...
/*******************************************************************************
* NAME:              DescUudtNetCanTxConfirmation
*
* CALLED BY:         DescUudtNetCAN
* PRECONDITIONS:
*
...
*
*******************************************************************************/
static void DescUudtNetCanTxConfirmation(CAN_MSG_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status)
{
  if(status == kDescUudtNetworkOk)
  {
    g_descUudtNetCanMsgManager[CAN_MSG_PARAM_VALUE].engine = kDescUudtNetCanEngineSentOk;
  }
  else /* status == kDescUudtNetworkFailed */
  {
    /* Signal unsuccessful sending */
    g_descUudtNetCanMsgManager[CAN_MSG_PARAM_VALUE].engine = kDescUudtNetCanEngineSentNotOk;
  }
}
...
*
*******************************************************************************/
static void DescUudtNetCanTxMsgEnd(CAN_MSG_FORMAL_PARAM_DEF_FIRST t_descUudtNetResult status)
{
  /* Release CAN messages */
  DescUudtNetInitCanMsgManager(CAN_MSG_PARAM_USAGE(DescUudtNetMsgRef(CAN_MSG_PARAM_VALUE)));

  /* Notify resource manager */
  DescUudtNetResourceMgrConfirmation(RESOURCE_PARAM_USAGE_FIRST(DescUudtNetResHandleRef(CAN_MSG_PARAM_VALUE)) status);

#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
  /* Decrement count of used CAN messages */
  g_descUudtNetCanMsgCounter--;
#endif
}
...
DESCNET_UUDT_STATIC void DescUudtNetCANReleaseResource(t_descUudtNetInfoPoolPtr infoPool)
{
  ...
  DescUudtNetInitResourceManager(RESOURCE_PARAM_USAGE_ONLY(infoPool->busHandle));
}
...
DESCNET_UUDT_STATIC void DescUudtNetCANCancelAllResponses(void)
{
  ...
  while(RESOURCE_ITER_VALUE != 0)
  {
    ...
    if(g_descUudtNetResourceManager[RESOURCE_ITER_VALUE].engine == kDescUudtNetCanResEngineWaitForConf)
    {
      /* Message obsolete */
      DescUudtNetLL_CancelTransmit(DescUudtNetMsgHandle(DescUudtNetMsgRef(RESOURCE_ITER_VALUE)));
      /* Notify by success, since the tester wanted no more messages to be sent (or timeout)*/
      DescUudtNetCanTxConfirmation(RESOURCE_ITER_USAGE_FIRST(RESOURCE_ITER_VALUE) kDescUudtNetworkOk);
    }
    RESOURCE_ITER_VALUE_DECREMENT;
  }
  DescUudtNetLL_InterruptRestore();
...
#endif
    {
      DescUudtNetCanTxConfirmation(CAN_MSG_ITER_USAGE_FIRST(CAN_MSG_ITER_VALUE) kDescUudtNetworkOk);
#if (kDescUudtNetMaxNumCanMsgsPerChannel > 1)
      /* Break loop */
...
void DescTransmitSingleFrame(DescMsg resData, vuint8 resLen)
{
  ...
  g_descUsdtNetSpontanResInfoPool.busInfo.busType = kDescUsdtNetBusTypeIsoTp;
  g_descUsdtNetSpontanResInfoPool.busInfo.comChannel = kDescOemSpontanResComChannel;
...
/*******************************************************************************
* NAME:              DescOemPostDisableNormalCommunication
*
* CALLED BY:         DescDispatcher
* PRECONDITIONS:     
*
* DESCRIPTION:       Reload the Tester Present timer.
*
*******************************************************************************/
static void DescOemPostDisableNormalCommunication(DESC_CONTEXT_FORMAL_PARAM_DEF_FIRST vuint8 status)
{
  /* Avoid warnings */
  DESC_CONTEXT_PARAM_DUMMY_USE;
...
#if defined(DESC_ENABLE_SID_28_APPL_POST_HANDLER)
  ApplDescPostDisableNormalCommunication(DESC_CONTEXT_PARAM_FIRST status);
#endif
}
...
* PRECONDITIONS:     
*
* DESCRIPTION:       Make preparations for enabling the programming mode.
*
*******************************************************************************/
...
#endif
  {
    /* Is access to the com bus allowed and resource is free and no DPID is currently read*/
    if((g_descOemStateCtrl.isUudtProcessSuspended == 0)&&
       (g_descDpidUudtResource == V_NULL)
#if defined(DESC_ENABLE_DYN_DEFINED_DPID_MODE)
