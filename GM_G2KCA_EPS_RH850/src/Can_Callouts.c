...
STATIC VAR(uint32, Can_Callouts_VAR) Bus2OffRecStartTime_M;
...
void ApplCanTimerStart( CanChannelHandle Channel,  vuint8 timerIdentification )
{
	...
	else if (timerIdentification == kCanLoopBusOffRecovery)
	{
		...
		else if (Channel == kCanChannel_CE)
		{
			GetRefTmr1MicroSec32bit_Oper(&Bus1OffRecStartTime_M);
		}
        else
        {
			GetRefTmr1MicroSec32bit_Oper(&Bus2OffRecStartTime_M);            
...
vuint8 ApplCanTimerLoop(CanChannelHandle Channel, vuint8 timerIdentification )
{
	...
	VAR(uint32, AUTOMATIC) Bus2OffRecDeltaTime_uS_T_u32;
	...
	else if (timerIdentification == kCanLoopBusOffRecovery)
	{
        ...
        {
			GetTiSpan1MicroSec32bit_Oper(Bus2OffRecStartTime_M, &Bus2OffRecDeltaTime_uS_T_u32);
			if (Bus2OffRecDeltaTime_uS_T_u32 >= 30U)
			{
				RtnVal_Cnt_T_u08 = FALSE;
			}
		}
...
/**********************************************************************************************************************
  * Name:        ApplCanCancelNotification
  * Description:
  *
  * Inputs:      txHandle - 
  *              
