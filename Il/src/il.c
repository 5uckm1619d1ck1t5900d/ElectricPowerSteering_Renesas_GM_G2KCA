/* Kernbauer Version: 1.14 Konfiguration: Il_Vector_Gm Erzeugungsgangnummer: 680 */
...
-----------------------------------------------------------------------------------------------------------------------
  \par      copyright
  \verbatim
  Copyright (c) 2000-2015 by Vector Informatik GmbH. All rights reserved.
...
-----------------------------------------------------------------------------------------------------------------------
  ...
  01.01.02  2015-01-29  Ms      ESCAN00080634 QNX only: Memory is overwritten during the context switch
  01.01.03  2015-05-22  Ms      ESCAN00083127 Update Il_Vector Core to Package 6.09.03
  01.02.00  2015-09-23  Ms      ESCAN00085433 Update Il_Vector Core to Package 6.10.00
            2015-09-23  Ms      ESCAN00085427 C_COMP_TI_TMS320_DCAN is not supported
**********************************************************************************************************************/
#endif /* IL_TYPE_GM */
...
#endif
#if defined ( IL_TYPE_GM )
# if( IL_VECTOR_GM_VERSION != 0x0102 )
#  error "Source and Header file are inconsistent!"
# endif
# if( IL_VECTOR_GM_RELEASE_VERSION != 0x00 )
#  error "Source and Header file are inconsistent!"
# endif
...
#   endif
    {
          ...
          0)
      {
        ...
        IlGetRxIndicationFlags(IlGetRxIndicationOffset(canRxHnd)) &=
          IL_CAST2UINT8(~IlGetRxIndicationMask(canRxHnd));
        CanInterruptRestore();
      ...
      if (cth != kCanTxHandleNotUsed)
      {
        ...
        if ((IlGetTxConfirmationFlags(IlGetTxConfirmationOffset(cth)) & IlGetTxConfirmationMask(cth)) != 0)
        {
          ...
          IlGetTxConfirmationFlags(IlGetTxConfirmationOffset(cth)) &=
            IL_CAST2UINT8(~IlGetTxConfirmationMask(cth));
...
static vuint8 IlProcessLearnedSourceAddress(IL_CHANNEL_ILTYPE_FIRST vuint8 srcAddress)
{
  ...
  else
  {
    ...
    if ((ilNodeCommActiveTimeoutFlags[IL_CHANNEL_ILPARA_MACRO][ilVnIndex] & ilVnMask) != 0)
    {
      ...
      ilNodeCommActiveTimeoutFlags[IL_CHANNEL_ILPARA_MACRO][ilVnIndex] &= IL_CAST2UINT8(~ilVnMask);
...
#  endif
  {
    ...
    for (ilBitIndex = 0; ilBitIndex < 8; ilBitIndex++)
    {
      if ((ilVnMsgMask & ilRxPackedMask) != 0)
      {
        ...
        ilVnMsgMask &= IL_CAST2UINT8(~ilRxPackedMask);
...
#  endif
  {
    ...
    for (ilBitIndex = 0; ilBitIndex < 8; ilBitIndex++)
    {
      ...
      {
        /* Clear this flag */
        ilVnMsgMask &= IL_CAST2UINT8(~ilTxPackedMask);
        /*--- Queue message On-Init only if it is configured to be allowed -----*/
        if ((ilTxState[ilTxHnd] & kTxQueueInit) != 0)
        {
      ...
      {
        ilTxPackedMask = IL_CAST2UINT8(ilTxPackedMask << 1);
      }
      ilTxHnd++;
    }
  }
...
#  endif
  ...
  {
    ilVnTxMessageEnable[(ilTxHnd >> 3)] &= IL_CAST2UINT8(~LSB2MSBBitMask[(ilTxHnd & 0x07)]);
  }
  return IL_OK;
...
#  endif
  ...
  {
    ilVnRxMessageEnable[(ilRxHnd >> 3)] &= IL_CAST2UINT8(~LSB2MSBBitMask[(ilRxHnd & 0x07)]);
  }
  return IL_OK;
