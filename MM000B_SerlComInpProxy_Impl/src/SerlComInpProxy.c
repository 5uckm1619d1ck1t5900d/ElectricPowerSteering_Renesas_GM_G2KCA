...
* --------  -------  --------  ---------------------------------------------------------------------------  ----------
* 02/03/17  1        GMN       Initial file creation from T1xx version 5                                    EA4#9617
***********************************************************************************************************************/
/**********************************************************************************************************************
...
static CONST(uint8, AUTOMATIC) SerlComInpProxyGmLanId[12u] = { /* PRQA S 3218 */
	2U, 3U, 0U, 18U, 18U, 119U, 3U, 3U, 0U, 18U, 18U, 119U
};
 ...
 *********************************************************************************************************************/
	...
	{
		GmLanId[Index_Cnt_T_u08] = SerlComInpProxyGmLanId[Index_Cnt_T_u08]; /* PRQA S 0492 */
	}
 ...
 *********************************************************************************************************************/
	...
	if (PPEI_NonDrivn_Whl_Rotationl_Stat_ind_b != 0U)
	{
	...
	if (PPEI_Engine_General_Status_1_ind_b != 0U)
	{
	...
	if (Antilock_Brake_and_TC_Status_HS_ind_b != 0U)
	{
	...
	if (LKA_Steering_Torque_Cmd_HS_ind_b != 0U)
	{
	...
	if (PPEI_Chassis_General_Status_1_ind_b != 0U)
	{
	...
	if (PPEI_Platform_General_Status_ind_b != 0U)
	{
	...
	if (PPEI_Trans_General_Status_2_ind_b != 0U)
	{
	...
	if (Brake_Pedal_Driver_Status_HS_ind_b != 0U)
	{
	...
	if (Driving_Mode_Control_HS_ind_b != 0U)
	{
	...
	if (Driven_Wheel_Grnd_Velocity_HS_ind_b != 0U)
	{
	...
	if (NonDriven_Wheel_Grnd_Velocity_HS_ind_b != 0U)
	{
	...
	if (PPEI_Platform_Eng_Cntrl_Requests_ind_b != 0U)
	{
	...
	if (Steering_Torque_Command_CE_ind_b != 0U)
	{
	...
	if (Vehicle_Dynamics_ESC_Hyb_CE_ind_b != 0U)
	{
	...
	if (Park_Assist_Parallel_CE_ind_b != 0U)
	{
	...
	if (Driven_Wheel_Grnd_Velocity_CE_ind_b != 0U)
	{
	...
	if (NonDriven_Wheel_Grnd_Velocity_CE_ind_b != 0U)
	{
	...
	if (PPEI_Engine_General_Status_5_ind_b != 0U)
	{
	...
	(void)CanRxFullCANObjTask((CanChannelHandle)CanRxSG1_Msg_EPS_B_HW_CHANNEL, (CanObjectHandle)CanRxSG1_Msg_EPS_B_HW_OBJ);
	(void)Rte_Call_SetRxSigGroup_Oper(IMCARBN_ECUIDSIGNALGROUP10MS_CNT_U08, IMCARBNRXDATASRC_SECDRY, &SG1_Msg_EPS_B._c[0]);
	if (SG1_Msg_EPS_B_ind_b != 0U)
	{
		CanWriteSyncSG1_Msg_EPS_B_ind_b(0U);
	}
#endif

#if defined(DUALECUSELECT_EPSB_SELECTED)
	(void)CanRxFullCANObjTask((CanChannelHandle)CanRxSG1_Msg_EPS_A_HW_CHANNEL, (CanObjectHandle)CanRxSG1_Msg_EPS_A_HW_OBJ);
	(void)Rte_Call_SetRxSigGroup_Oper(IMCARBN_ECUIDSIGNALGROUP10MS_CNT_U08, IMCARBNRXDATASRC_SECDRY, &SG1_Msg_EPS_A._c[0]);
	if (SG1_Msg_EPS_A_ind_b != 0U)
	{
		CanWriteSyncSG1_Msg_EPS_A_ind_b(0U);
	}
#endif
