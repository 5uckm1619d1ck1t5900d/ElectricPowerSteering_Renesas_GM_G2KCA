 ...
 *********************************************************************************************************************/
	...
	*Rte_Pim_EveCnt() = 0U;
	for (EveId_Cnt_T_u08 = 1U; EveId_Cnt_T_u08 < Dem_Cfg_GlobalEventCount(); EveId_Cnt_T_u08 += 1U)
	{
...
static FUNC(void, GmMsg778BusHiSpd_CODE) SetFaildEve(void)
{
	...
	if ((*Rte_Pim_ActvEveId() != INVLDEVEID_CNT_U08) && (SysSt_Cnt_T_enum != SYSST_OFF))
	{
		...
		{
			/* Check the CONFIRMED bit */
			if (GetBit(DtcSts_Cnt_T_u08, 3U) == 0U)
			{
				/* When the CTC is no longer CONFIRMED then it has aged and can be ignored - set numbers to zeros */
				DtcNr_Cnt_T_u16 = 0U;
				DtcFailrTyp_Cnt_T_u08 = 0U;
			}
			else
			{
