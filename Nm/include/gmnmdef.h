...
-----------------------------------------------------------------------------------------------------------------------
  ...
  4.04.00   2015-01-19  vismpf  ESCAN00080646 Added context switch support for QNX
                                ESCAN00080710 no changes in this file
  4.04.01   2015-09-19  visoh   ESCAN00085301 no changes in this file
  4.04.02   2015-09-26  vismpf  ESCAN00085504 no changes in this file
  4.04.03   2015-04-16  vismpf  ESCAN00059402 no changes in this file
                                ESCAN00082288 no changes in this file
                                ESCAN00085356 no changes in this file
                                ESCAN00085357 no changes in this file
                                ESCAN00087109 added compile error check
**********************************************************************************************************************/
...
#define NM_GMLAN_GM_VERSION                     0x0404
#define NM_GMLAN_GM_RELEASE_VERSION             0x03
...
#if ( ( defined VQNX_ENABLE_CONTEXT_SWITCH ) && ( defined( VGEN_ENABLE_QWRAP ) || defined( VGEN_ENABLE_MDWRAP ) ) )
/*
 * Name:    tNmModuleContextStruct
 * Used by: NmGetModuleContext() and NmSetModuleContext() API
 * Notes:   ESCAN00080646: QNX data structure which holds all variables of the NM
 */
typedef struct tsNmModuleContextStruct
{
  ...
  tCommEnableTimer  nmGenericTimer_CS[kNmNumberOfChannels];
  vuint16           nmBusOffRecoveryTimeCnt_CS[kNmNumberOfChannels];
  nmActionType      nmAction_CS[kNmNumberOfChannels];
  vuint8            nmSendInitVnmfCnt_CS[kNmNumberOfChannels];

# if defined(NM_ENABLE_INDEXED_NM)
  nmStateType  nmState_CS[kNmNumberOfChannels];
# else
  nmStateType  nmState_CS;
# endif

# if defined( NM_ENABLE_VN_INITACTIVE )
  vuint8  nmInitVnCnt_CS[kNmNumberOfChannels];
# endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
  tNmWakeupCnt nmWakeupCnt_CS[kNmNumberOfChannels];
  vuint16      nmSendContVnmfCnt_CS[kNmNumberOfChannels];
  vuint8       nmNrOfFailedWakeup_CS[kNmNumberOfChannels];
  vuint8       nmLastVnmfType_CS[kNmNumberOfChannels];
# endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED ) || defined ( NM_ENABLE_HIGHSPEED_MODE )
  vuint8  nmTxConfirmCnt_CS[kNmNumberOfChannels];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
  vuint16 nmFdrNwActiveCnt_CS[kNmNumberOfChannels];
  vuint16 nmFdrCommEnableTimer_CS[kNmNumberOfChannels];
  vuint8  nmFdrHLVWRxTxFlag_CS[kNmNumberOfChannels];
  vuint8  nmFdrChannelState_CS[kNmNumberOfChannels];
...
#   if (defined ( NM_ENABLE_FDR_NO_SLEEP_CONF_FAULT_REPORT )) || \
  ...
  vuint8 nmFdrApplNotConfirmedSleepCnt_CS[kNmNumberOfChannels];
#   endif
#  endif
# endif

  /* VN related data */
  vuint16     nmVnSpvStabilityTimer_CS[kNmNumberOfVirtualNets];
  tNmVnTimer  nmVnTimer_CS[kNmNumberOfVirtualNets];
  vuint8      nmVnState_CS[kNmNumberOfVirtualNets];
# if defined( NM_ENABLE_VN_REMOTED )
  vuint8      nmVnStateInhibit_CS[kNmNumberOfVirtualNets];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
  vuint16 nmFdrVnActiveCnt_CS[kNmNumberOfVirtualNets];
  vuint8  nmFdrVnStateMitigated_CS[kNmNumberOfVirtualNets];
#  if (defined ( NM_ENABLE_VN_ACTIVATOR )) || (defined ( NM_ENABLE_VN_LOCAL )) || (defined ( NM_ENABLE_VN_REMOTED ))
  vuint8 nmFdrVnActivationState_CS[kNmNumberOfVirtualNets];
#  endif
# endif

  /* VNMF msg related data */
  nmMessType nmSendMess_CS;
# if defined( NM_ENABLE_INHIBIT_VN_HIGHLOAD ) && defined( NM_ENABLE_VN_ACTIVATOR )
  vuint8 nmVnQueued_CS[kNmNumberOfVnBytes];
# endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED  )
  vuint8 nmSendInit_CS[kNmNumberOfVnBytes];
# endif
# if defined( NM_ENABLE_VN_ACTIVATOR)
  vuint8 nmSendContinue_CS[kNmNumberOfVnBytes];
# endif
...
/*--- ESCAN00080646: QNX context switch support ------------------------------------------------------------------------------------*/
...
#if ( defined ( NM_ENABLE_RECOVER_VN_AFTER_BUSOFF ) )
/* ESCAN00087109 */
# error "Nm_Gmlan_Gm (config): NM_ENABLE_RECOVER_VN_AFTER_BUSOFF is no longer supported and must not be defined."
#endif
