...
-----------------------------------------------------------------------------------------------------------------------
  ...
  4.04.00   2015-01-19  vismpf  ESCAN00080646 Added context switch support for QNX
                                ESCAN00080710 Adapted encapsulation of variable channel
  4.04.01   2015-09-19  visoh   ESCAN00085301 Reception of signals for remotely activated VN is not possible
  4.04.02   2015-09-26  vismpf  ESCAN00085504 locked interrupts around IlRxDeactivateVn() in NmTimerTask()
  4.04.03   2015-04-16  vismpf  ESCAN00059402 Added encapsulation for not needed function
                                ESCAN00082288 Adapted condition to call ApplNwmVnActivationFailed
                                ESCAN00085356 Adapted condition to call assertion
                                ESCAN00085357 Don't delete bits related to remote VN activation when vnTimer times out
                                ESCAN00087109 no changes in this file
**********************************************************************************************************************/
...
#if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED ) || defined( NM_ENABLE_HIGHSPEED_MODE )
/*
 * Name:        nmTxConfirmCnt
...
# endif
        {
          ...
          if( nmFdrVnStateMitigated[fdrVnHandle] == 0 )
          {
            ...
            if( nmFdrVnActiveCnt[fdrVnHandle] >= NmCfg_FdrVnActive_Cnt(channel) )
            {
              ...
              /* ESCAN00085504: Added ISR locks around IlRxDeactivateVnMsg */
              CanInterruptDisable();
...
# if defined( NM_ENABLE_LOWVOLTAGE_MODE ) && \
     defined( NM_ENABLE_CANOFF_ON_LOWVOLTAGE_MODE )
      if( (nmAction[NM_CHANNEL_IDX] != NM_BUSOFF) && (nmLowVoltageMode == kNmFalse) )
# else
...
# endif
  ...
  if( nmHLVWTxTransmitted == kNmTrue )
  {
...
# if defined( NM_ENABLE_HIGHSPEED_MODE )
  ...
  /* ESCAN00029370 */
  if( (nmHighSpeedMode == kNmTrue) && (nmInternalHighSpeedMode == kNmTrue) )
  {
    return( NM_ERROR );
  }
# endif
...
vuint8 NmHVPreTransmit( CanTxInfoStruct ctis )
{
  /* ESCAN00080710: Adapted encapsulation of variable channel */
# if ( defined ( NM_ENABLE_INDEXED_NM ) ) && \
     ( defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED ) ) && \
     ( defined( NM_ENABLE_SINGLEWIRE_TRCV ) || defined( NM_ENABLE_HLVW_INDICATION_FCT ) )
...
/* ESCAN00059402: Added encapsulation for not needed functions */
#if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
/**********************************************************************************************************************
  Name:         NmPreTransmit
...
#endif /* NM_ENABLE_VN_ACTIVATOR || NM_ENABLE_VN_REMOTED */

/* ESCAN00059402: Added encapsulation for not needed function */
#if defined( NM_ENABLE_VN_REMOTED )
/**********************************************************************************************************************
  Name:         NmPrecopy
  ...
  for( vnHndl=(vsint8)NmCfg_VN_StopIndex(channel); vnHndl>=(vsint8)NmCfg_VN_StartIndex(channel); vnHndl-- )
  {
    ...
    else
    {
      if( (nmVnState[vnHndl] & NmVnActive) != 0 )
      {
        ...
        {
#if defined( NM_ENABLE_VN_ACTIVATOR )
          /* ESCAN00082288: NmVnActivatorPending bit is set indicates that VN is re-activated and NM waits for VNMF confirmation or first VNMF timeout.
           * ApplNwmVnActivationFailed should only be called if there was already a VNMF timeout.
           * ApplNwmVnActivationFailed should also not be called if activation of init active VNs is pending ( nmInitVnTx == kNmTrue ). */
          if( ( ( nmVnState[vnHndl] & ( NmVnActivator|NmVnActivatorPending ) ) == NmVnActivator ) &&
              ( nmInitVnTx == kNmFalse ) )
          {
...
# endif
        ...
        if( nmVnTimer[vnHndl]==0 )
        {
          ...
          {
            /* reset VN state */
            /* ESCAN00085357: Keep bits related to remote activation in case VNMF
             * is received within this task function call. */
            nmVnState[vnHndl] &= (vuint8)(kNmVnRemotedPending | kNmVnRestartTimer | kNmVnRemoteInit);
          }
#endif
          nmVnState[vnHndl] &= (vuint8)(~(vuintx)(NmVnActive));
          nmRescanVn = kNmTrue;

          /* ESCAN00085356: If a VNMF message is received within this task function call,
           * also kNmVnRemotedPending bit would still be set which is also ok. */
          NmAssert( ( ((nmVnState[vnHndl] & (NmVnActivatorPending | kNmVnRemotedPending)) != 0 ) || ( nmVnState[vnHndl] == 0 ) ), NMERR_VNSTATE_INCONSISTENT )/*;*/
          CanInterruptRestore();
...
} /* NmVnmfRecoveryFromBusoff() */
#endif /* NM_ENABLE_VN_ACTIVATOR && NM_ENABLE_RECOVER_VN_AFTER_BUSOFF */

#if ( defined ( VGEN_ENABLE_MDWRAP ) && defined ( VQNX_ENABLE_CONTEXT_SWITCH ) )
/*****************************************************************************
**  Function:     NmGetModuleContext
**
**  Called by:    QNX handler which switches the context from the mini to the
**                full driver of QNX
**
**  Parameter:    tNmModuleContextStructPtr pContext - pointer to structure
**                which needs to be filled by this function.
**
**  Returncode:   -
**
**  Description:  This function copies all internal and global variables of the
**                network managment into the passed structure.
**
******************************************************************************/
void NmGetModuleContext( tNmModuleContextStructPtr pContext )
{
  ...
  pContext->nmMagicNumber = (vuint32)((((vuint32)NM_GMLAN_GM_VERSION)<<8) |
                                       ((vuint32)NM_GMLAN_GM_RELEASE_VERSION));

  /* Channel related data */
# if defined(NM_ENABLE_INDEXED_NM)
  for( index=0; index < kNmNumberOfChannels; index++ )
# else
  index=0;
# endif
  {
    pContext->nmGenericTimer_CS[index]           = nmGenericTimer[index];
    pContext->nmBusOffRecoveryTimeCnt_CS[index]  = nmBusOffRecoveryTimeCnt[index];
    pContext->nmAction_CS[index]                 = nmAction[index];
    pContext->nmSendInitVnmfCnt_CS[index]        = nmSendInitVnmfCnt[index];

#if defined(NM_ENABLE_INDEXED_NM)
    pContext->nmState_CS[index].c[0] = nmState[index].c[0];
    pContext->nmState_CS[index].c[1] = nmState[index].c[1];
    pContext->nmState_CS[index].c[2] = nmState[index].c[2];
    pContext->nmState_CS[index].c[3] = nmState[index].c[3];
#else
    pContext->nmState_CS.c[0] = nmState.c[0];
    pContext->nmState_CS.c[1] = nmState.c[1];
    pContext->nmState_CS.c[2] = nmState.c[2];
    pContext->nmState_CS.c[3] = nmState.c[3];
#endif

#if defined( NM_ENABLE_VN_INITACTIVE )
    pContext->nmInitVnCnt_CS[index]        = nmInitVnCnt[index];
#endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
    pContext->nmWakeupCnt_CS[index]        = nmWakeupCnt[index];
    pContext->nmSendContVnmfCnt_CS[index]  = nmSendContVnmfCnt[index];
    pContext->nmNrOfFailedWakeup_CS[index] = nmNrOfFailedWakeup[index];
    pContext->nmLastVnmfType_CS[index]     = nmLastVnmfType[index];
#endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED ) || defined ( NM_ENABLE_HIGHSPEED_MODE )
    pContext->nmTxConfirmCnt_CS[index]     = nmTxConfirmCnt[index];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
    pContext->nmFdrNwActiveCnt_CS[index]     = nmFdrNwActiveCnt[index];
    pContext->nmFdrCommEnableTimer_CS[index] = nmFdrCommEnableTimer[index];
    pContext->nmFdrHLVWRxTxFlag_CS[index]    = nmFdrHLVWRxTxFlag[index];
    pContext->nmFdrChannelState_CS[index]    = nmFdrChannelState[index];
#  if defined ( NM_ENABLE_SLEEPCONFIRMATION_FCT )
#   if (defined ( NM_ENABLE_FDR_NO_SLEEP_CONF_FAULT_REPORT )) || \
      (defined ( NM_ENABLE_FDR_NO_SLEEP_CONF_FAULT_MITIGATION ))
    pContext->nmFdrApplNotConfirmedSleepCnt_CS[index] = nmFdrApplNotConfirmedSleepCnt[index];
#   endif
#  endif
# endif
  }
  ...
  /* VN related data */
  for( index=0; index < kIlNumberOfVirtualNets; index++ )
  {
    pContext->nmVnSpvStabilityTimer_CS[index]  = nmVnSpvStabilityTimer[index];
    pContext->nmVnTimer_CS[index]              = nmVnTimer[index];
    pContext->nmVnState_CS[index]              = nmVnState[index];
# if defined( NM_ENABLE_VN_REMOTED )
    pContext->nmVnStateInhibit_CS[index]       = nmVnStateInhibit[index];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
    pContext->nmFdrVnActiveCnt_CS[index]       = nmFdrVnActiveCnt[index];
    pContext->nmFdrVnStateMitigated_CS[index]  = nmFdrVnStateMitigated[index];
#  if (defined ( NM_ENABLE_VN_ACTIVATOR )) || (defined ( NM_ENABLE_VN_LOCAL )) || (defined ( NM_ENABLE_VN_REMOTED ))
    pContext->nmFdrVnActivationState_CS[index] = nmFdrVnActivationState[index];
#  endif
# endif
  }

  /* VNMF msg related data */
  for( index=0; index < kNmNumberOfVnBytes; index++ )
  {
    pContext->nmSendMess_CS.c[index]   = nmSendMess.c[index];
#if defined( NM_ENABLE_INHIBIT_VN_HIGHLOAD ) && defined( NM_ENABLE_VN_ACTIVATOR )
    pContext->nmVnQueued_CS[index]     = nmVnQueued[index];
#endif
#if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
    pContext->nmSendInit_CS[index]     = nmSendInit[index];
#endif
#if defined( NM_ENABLE_VN_ACTIVATOR )
    pContext->nmSendContinue_CS[index] = nmSendContinue[index];
#endif
  }
} /* NmGetModuleContext() */
#endif /* NM_ENABLE_GET_CONTEXT || VGEN_ENABLE_MDWRAP */

#if ( defined ( VGEN_ENABLE_QWRAP ) && defined ( VQNX_ENABLE_CONTEXT_SWITCH ) )
/*****************************************************************************
**  Function:    NmSetModuleContext
**
**  Called by:   QNX handler which switches the context from the mini to the
**               full driver of QNX.
**
**  Parameter:   tNmModuleContextStructPtr pContext - pointer to structure
**               which this function will copy data from.
**
**  Returncode:  1: The magicNumber member of the struct parameter
**                  matches the version of the modules.
**               0: The magicNumber does not match.
**
**  Description: This function intitialises all internal and global variables
**               of the network management from the passed structure.
**
******************************************************************************/
vuint8 NmSetModuleContext( tNmModuleContextStructPtr pContext ) /* PRQA S 3673 */ /* MD_NmGmlanGm_3673 */
{
  vuint16 index;
  vuint8 retVal = 0u;

  if( pContext->nmMagicNumber == (vuint32)((((vuint32)NM_GMLAN_GM_VERSION)<<8) |
                                            ((vuint32)NM_GMLAN_GM_RELEASE_VERSION)) )
  {
    retVal = 1u;
...
# endif
    {
      nmGenericTimer[index]           = pContext->nmGenericTimer_CS[index];
      nmBusOffRecoveryTimeCnt[index]  = pContext->nmBusOffRecoveryTimeCnt_CS[index];
      nmAction[index]                 = pContext->nmAction_CS[index];
      nmSendInitVnmfCnt[index]        = pContext->nmSendInitVnmfCnt_CS[index];

#if defined(NM_ENABLE_INDEXED_NM)
      nmState[index].c[0]       = pContext->nmState_CS[index].c[0];
      nmState[index].c[1]       = pContext->nmState_CS[index].c[1];
      nmState[index].c[2]       = pContext->nmState_CS[index].c[2];
      nmState[index].c[3]       = pContext->nmState_CS[index].c[3];
#else
      nmState.c[0] = pContext->nmState_CS.c[0];
      nmState.c[1] = pContext->nmState_CS.c[1];
      nmState.c[2] = pContext->nmState_CS.c[2];
      nmState.c[3] = pContext->nmState_CS.c[3];
#endif

#if defined( NM_ENABLE_VN_INITACTIVE )
      nmInitVnCnt[index]        = pContext->nmInitVnCnt_CS[index];
#endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
      nmWakeupCnt[index]        = pContext->nmWakeupCnt_CS[index];
      nmSendContVnmfCnt[index]  = pContext->nmSendContVnmfCnt_CS[index];
      nmNrOfFailedWakeup[index] = pContext->nmNrOfFailedWakeup_CS[index];
      nmLastVnmfType[index]     = pContext->nmLastVnmfType_CS[index];
#endif
# if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED ) || defined ( NM_ENABLE_HIGHSPEED_MODE )
      nmTxConfirmCnt[index]     = pContext->nmTxConfirmCnt_CS[index];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
      nmFdrNwActiveCnt[index]     = pContext->nmFdrNwActiveCnt_CS[index];
      nmFdrCommEnableTimer[index] = pContext->nmFdrCommEnableTimer_CS[index];
      nmFdrHLVWRxTxFlag[index]    = pContext->nmFdrHLVWRxTxFlag_CS[index];
      nmFdrChannelState[index]    = pContext->nmFdrChannelState_CS[index];
#  if defined ( NM_ENABLE_SLEEPCONFIRMATION_FCT )
#   if (defined ( NM_ENABLE_FDR_NO_SLEEP_CONF_FAULT_REPORT )) || \
       (defined ( NM_ENABLE_FDR_NO_SLEEP_CONF_FAULT_MITIGATION ))
      nmFdrApplNotConfirmedSleepCnt[index] = pContext->nmFdrApplNotConfirmedSleepCnt_CS[index];
#   endif
#  endif
# endif
    }
    ...
    for( index=0; index < kIlNumberOfVirtualNets; index++ )
    {
      nmVnSpvStabilityTimer[index]  = pContext->nmVnSpvStabilityTimer_CS[index];
      nmVnTimer[index]              = pContext->nmVnTimer_CS[index];
      nmVnState[index]              = pContext->nmVnState_CS[index];
# if defined( NM_ENABLE_VN_REMOTED )
      nmVnStateInhibit[index]       = pContext->nmVnStateInhibit_CS[index];
# endif

# if defined ( NM_ENABLE_FDR_ALGORITHM )
      nmFdrVnActiveCnt[index]       = pContext->nmFdrVnActiveCnt_CS[index];
      nmFdrVnStateMitigated[index]  = pContext->nmFdrVnStateMitigated_CS[index];
#  if (defined ( NM_ENABLE_VN_ACTIVATOR )) || (defined ( NM_ENABLE_VN_LOCAL )) || (defined ( NM_ENABLE_VN_REMOTED ))
      nmFdrVnActivationState[index] = pContext->nmFdrVnActivationState_CS[index];
#  endif
# endif
    }
    ...
    for( index=0; index < kNmNumberOfVnBytes; index++ )
    {
      nmSendMess.c[index]   = pContext->nmSendMess_CS.c[index];
#if defined( NM_ENABLE_INHIBIT_VN_HIGHLOAD ) && defined( NM_ENABLE_VN_ACTIVATOR )
      nmVnQueued[index]     = pContext->nmVnQueued_CS[index];
#endif
#if defined( NM_ENABLE_VN_ACTIVATOR ) || defined( NM_ENABLE_VN_REMOTED )
      nmSendInit[index]     = pContext->nmSendInit_CS[index];
#endif
#if defined( NM_ENABLE_VN_ACTIVATOR )
      nmSendContinue[index] = pContext->nmSendContinue_CS[index];
#endif
    }
  }
  return retVal;
} /* NmSetModuleContext() */
#endif /* NM_ENABLE_SET_CONTEXT || VGEN_ENABLE_QWRAP */
...
/* Module specific MISRA deviations:
   ...
   MD_NmGmlanGm_3453:
     ...
     Prevention:  Cover by code review
   MD_NmGmlanGm_3673:
     Reason:      Prototype can't be changed because defined by other module
     Risk:        N/A
     Prevention:  N/A
   MD_NmGmlanGm_4700:
