...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActRestoreRomDefaults(void)
{
    /* create a dummy CRC recalculation job (no RAM block) --> ensure that no CRC calculation needs to be performed  *
    * Otherwise, such a job might still be pending. Initiated during read (CRC check), but read failed              */
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishRestoreRomDefaults(void)
{
    /* we got some Default Data, is it the permanent RAM Block to be validated? */
    if((NvM_CurrentJob_t.RamAddr_t == NULL_PTR) && 
        (NvM_CurrentBlockInfo_t.Descriptor_pt->RomBlockDataAddr_pt != NULL_PTR)) /* COV_NVM_APICFGCLASS */
    {
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActCopyNvDataToBuf(void)
{
#if(STD_ON == NVM_CRC_INT_BUFFER) /* ECO_IGNORE_LINE */
    ...
    { /* Copy to buffer is pending */
        /* call callback NvMWriteRamBlockTo NvM */
        ...
        { /* Neither explicit sync mechanism, nor CRC */
            NvM_CurrentBlockInfo_t.ByteCount_u16 = 0;
        }
    }
    else
