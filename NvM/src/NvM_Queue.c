    ...
    )
{
  ...
  /* Translate internal ServiceId into public/official one */
  const NvM_ServiceIdType PublicServiceId = NvM_IntServiceDescrTable_at[ServiceId].PublicFid_t;
    ...
    /* queue job only if it isn't already queued */
    if(NvM_RamMngmt_ptloc->NvRamErrorStatus_u8 != NVM_REQ_PENDING)
    {
        ...
        if(UsedQueue->EmptyList != NVM_LIST_END)
        {
            ...
            retVal = TRUE;
            NvM_RamMngmt_ptloc->NvRamErrorStatus_u8 = NVM_REQ_PENDING; /* SBSW_NvM_AccessBlockManagementArea */

            /* Leave Critical section here. Avoiding calling DEM inside a critical section  *
             * prevents us from having a "central" exit at function's end                   */
            NvM_ExitCriticalSection();

            NvM_BlockNotification(BlockId, PublicServiceId, NVM_REQ_PENDING);
        }
        else
        {   /* no free element --> queuing failed */

            /* avoid calling DEM inside a critical section */
            NvM_ExitCriticalSection();

            /* call production error queueoverflow */
...
NVM_LOCAL FUNC(NvM_QueueEntryRefType, NVM_PRIVATE_CODE) NvM_QueueFindBlock(NvM_QueueEntryRefType QueueHead, NvM_BlockIdType BlockId)
{
    if(QueueHead != NVM_LIST_END)
    {
        ...
        do
        {
            ...
            {
                /* found the entry: immediately leave function */
                 return elem;
            }
        }
        while(elem != QueueHead);
    }
    /* if we get here, no queue element holding BlockId was found. */
    return NVM_LIST_END;
} /* PRQA S 2006 */ /* MD_MSR_14.7 */
