...
#define osDisableHighLevel()    osDisableLevel()
#define osEnableHighLevel()     osEnableLevel()
...
/* MISRA RULE 8.5 VIOLATION: osSetMpuConfig is an assembler macro which is used in
   several modules and therefore the definition has to be given in the header file. */
/* MISRA RULE 14.1 not violated: osSetMpuConfig is only used in some configurations.
 * As it is implemented as asm macro it does not harm in the other configurations. */
asm void osSetMpuConfig(osuint32 cfg_addr) /* PRQA S 1503 */
{                                          /* PRQA S 3406 */
%reg cfg_addr
...
#endif /* osdNumberOfDynamicMpuRegions > 0 */
