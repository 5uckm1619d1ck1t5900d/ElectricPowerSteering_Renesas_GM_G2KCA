...
#define osAddToTargetTIPTime(TIP, value, ctr) \
   IF2ANDCOVER(((oskCounterInternalMaximumValue[ctr] - osAlarmTime[TIP]) < (value)), (osAlarmTime[TIP] > osdCounterOverflowRange(ctr)),0x0454,0x0455,0x0456) \
...
static osqRAM0 osqRAM1 osqRAM2 TickType osqRAM3 osCalculateAbsTimes(CounterType CounterID, TickType AbsVal, TickType Now, TickType* Delta)
{
   ...
   IFCOVER( Now <= oskCounterMaxAllowedValue[CounterID],0x0440,0x0441)
   for(usernow = Now; usernow > oskCounterMaxAllowedValue[CounterID]; usernow -= (oskCounterMaxAllowedValue[CounterID] + 1)) 
   { 
      IFCOVER( (usernow - (oskCounterMaxAllowedValue[CounterID] + 1)) > oskCounterMaxAllowedValue[CounterID],0x0442,0x0443)
   }
   ...
   do
   {
      ...
      switch (react)
      {
         ...
         case osdSTReact_FinalDelay :
            IFCOVER(react==osdSTReact_Continue,0x0444,0x0445)
            IFCOVER(react==osdSTReact_InitialDelay,0x0446,0x0447)
            IFCOVER(react==osdSTReact_FinalDelay,0x0448,0x0449)
      ...
      COVER(0x03D5)
      IFCOVER(distToNextEP == 0,0x044A,0x044B)
...
osqFunc1 StatusType osqFunc2 osStopScheduleTable(ScheduleTableType ScheduleTableID) /* PRQA S 1503 */
{
   ...
   IF2ORCOVER( (osSTState[ScheduleTableID] == SCHEDULETABLE_STOPPED) , (osSTState[ScheduleTableID] == SCHEDULETABLE_NEXT),0x0457,0x0458,0x0459)
   if (
...
osqFunc1 StatusType osqFunc2 osNextScheduleTable(ScheduleTableType ScheduleTableID_current, ScheduleTableType ScheduleTableID_next) /* PRQA S 1503 */
{
   ...
   IF2ORCOVER( ((oskApplScheduleTableAccess[ScheduleTableID_current] & (1UL << osOwnCcb->LockIsNotNeeded.ossActiveApplicationID)) == 0)  , ((oskApplScheduleTableAccess[ScheduleTableID_next] & (1UL << osOwnCcb->LockIsNotNeeded.ossActiveApplicationID)) == 0),0x045A,0x045B,0x045C)
...
osqFunc1 void osqFunc2 osInitScheduleTables(void)
{
   ...
   {
      osAlarmTime[stID] = 0;
      IFCOVER( (stID+1) < (osrNumberOfAlarms + osrNumberOfScheduleTables),0x044C,0x044D)
   }
   ...
   for(ctr = (CounterType)0; ctr < osdNumberOfCounters; ctr++)
   {
      ...
      IFCOVER( (ctr+1) < osdNumberOfCounters,0x0452,0x0453)
   } /* for(ctr = (CounterType)0; ctr < osdNumberOfCounters; ctr++) */
#endif      
   COVER(0x043F)
}
