...
osqFunc1 StatusType osqFunc2 osGetAlarm(AlarmType alarmID, TickRefType diffTickTime)   /* PRQA S 1503 */
{
   ...
   if (osdTRUE == osdIsHiResCounter(CounterID)) 
   {
      ...
      IFCOVER(osAlarmTime[alarmID] <= oskCounterHalfrange[CounterID],0x01EE,0x01EF)
...
/* KB end osekHWosSetRelAlarmHead */
 {/* extra curly brace to allow coverage code above and variable declarations inside OS_SA_ENTRY below */
   ...
   COVER(0x01B5)
      ...
      if (cycle != 0)
      {
         IF2ORCOVER( (cycle > oskCounterMaxAllowedValue[CounterID]) , (cycle < oskCounterMinCycle[CounterID]),0x01F4,0x01F5,0x01F6)
         ...
         ELSEWITHCOVER(0x01BA)
      }
      ELSEWITHCOVER(0x01BB)
   ...
   IF2ANDCOVER(((oskCounterInternalMaximumValue[CounterID] - now) < deltaTicks) , (now > osdCounterOverflowRange(CounterID)),0x01F7,0x01F8,0x01F9)
 ...
 {/* extra curly brace to allow coverage code above and variable declarations inside OS_SL_ENTRY below */
  ...
  CounterID = oskAlarmCounterRef[alarmID];
   ...
   COVER(0x01CF)
      ...
      if (cycle != 0)
      {
         IF2ORCOVER( (cycle > oskCounterMaxAllowedValue[CounterID]) , (cycle < oskCounterMinCycle[CounterID]),0x01FA,0x01FB,0x01FC)
         if ((cycle > oskCounterMaxAllowedValue[CounterID])
             || (cycle < oskCounterMinCycle[CounterID]))
         {
            COVER(0x01D0)
   ...
   IFCOVER( now<=oskCounterMaxAllowedValue[CounterID],0x01F0,0x01F1)
   for(usernow = now; usernow > oskCounterMaxAllowedValue[CounterID]; usernow -= (oskCounterMaxAllowedValue[CounterID] + 1))
   {  
      ;
      IFCOVER( (usernow-(oskCounterMaxAllowedValue[CounterID] + 1)) > oskCounterMaxAllowedValue[CounterID],0x01F2,0x01F3)
   }
