...
osqFunc1 void osqFunc2 osInitINTC(void) /* PRQA S 1503 */
{
   ...
   COVER(0x00F0)   
         ...
         osSetSCBP((osuint32)&osSysCallTable);   
#if (osdNumberOfAllISRs > 0)
         {
            osuint16 index;
            IFCOVER(0 >= osdNumberOfAllISRs,0x0143,0x0144)      
...
#define osSetMPURegion(regnr) \
        osSetMPLA##regnr(osConfigBlock.osMPUConfig.osMPLA##regnr); \
        osSetMPUA##regnr(osConfigBlock.osMPUConfig.osMPUA##regnr); \
        osSetMPAT##regnr(osConfigBlock.osMPUConfig.osMPAT##regnr) /* PRQA S 881 */
...
static osqFunc1 osuint16 osqFunc2 osCRC16(const osuint8 *ptr, osuint16 length)
{
   ...
   for (index = 0; index < length; index++)
   {
      ...
      IFCOVER((index + 1) < length,0x0145,0x0146)
   }
...
osqFunc1 void osqFunc2 osStartOSc(void)     /* PRQA S 1503 *//* PRQA S 1505 */
{
   ...
   IF3ORCOVER( ((osActiveAppMode & ((AppModeType)(osActiveAppMode-((AppModeType)1)))) != 0), (osActiveAppMode == 0), (osActiveAppMode > (1U << (osrNumberOfAppModes - 1))) ,0x015F,0x0160,0x0161,0x0162)
...
/* TR:SPMF12:0006 End */
...

   {
      ...
      for (nr = 0; nr < osrNumberOfAllResources; nr++)
      {
         ...
         IFCOVER( (nr+1) < osrNumberOfAllResources,0x014B,0x014C)
      }
      COVER(0x0123)
   }
...

   {
      ...
      for(na = 0; na < osdNumberOfApplications; na++)
      {
         ...
         IFCOVER( (na+1) < osdNumberOfApplications,0x014D,0x014E)
      }
      /* TR:SASR40:0500 End */
      COVER(0x0125)
   }
   
   {
      ...
      for (nt = 0; nt < osrNumberOfAllTasks; nt++)
      {
         COVER(0x0126)
            ...
            ELSEWITHCOVER(0x012A)
         IFCOVER( (nt+1) < osrNumberOfAllTasks,0x014F,0x0150)
      }
      COVER(0x012B)
   }

   {
      ...
      for(nc = 0; nc < osrNumberOfCounters; nc++)
      {
         ...
         IFCOVER( (nc+1) < osrNumberOfCounters,0x0151,0x0152)
      }
      COVER(0x012D)
   }
   ...
   COVER(0x0131)
   {
      ...
      for (ni=0; ni<osdNumberOfCat2ISRs; ni++)
      {
         ...
         IFCOVER( (ni+1)<osdNumberOfCat2ISRs,0x0153,0x0154)
      }
      COVER(0x0133)
   }
   {
      ...
      for (nt=0; nt<osdNumberOfAllTasks; nt++)
      {
         ...
         IFCOVER( (nt+1)<osdNumberOfAllTasks,0x0155,0x0156)
      }
      COVER(0x0135)
   }
   {
      ...
      for (nr=0; nr<osdNumberOfAllResources; nr++)
      {
         ...
         IFCOVER( (nr+1)<osdNumberOfAllResources,0x0157,0x0158)
      }
      COVER(0x0137)
   }
...
static osqFunc1 void osqFunc2 osInitTCBs(void)
{
   ...
   for (nt=0; nt<osrNumberOfAllTasks; nt++)
   {
      ...
      IFCOVER( (nt+1)<osrNumberOfAllTasks,0x0159,0x015A)
   }
   COVER(0x013F)
   ...
   for (np=0; np<osrRTSize; np++)
   {
      ...
      IFCOVER( (np+1)<osrRTSize,0x015B,0x015C)
   }
   COVER(0x0141)
   ...
   for (np = 0; np < osrNumberOfPriorities; np++)
   {
      ...
      IFCOVER( (np+1) < osrNumberOfPriorities,0x015D,0x015E)
   } /* for (np = 0; np < osrNumberOfPriorities; np++) */
