 ...
 *  Generated at:  Mon Sep 26 14:16:37 2016
 *
...
#define SCAGFACVEHLATA_MTRPERSECSQD_F32	9.81F
...
static FUNC(void, SysFricLrng_CODE) HwVelConstraint(VAR(float32,AUTOMATIC) FilHwVel_HwRadPerSec_T_f32, P2VAR(boolean, AUTOMATIC, SysFricLrng_VAR)HwVelOK_Cnt_T_Logl,
 ...
 *********************************************************************************************************************/
	...
	*Rte_Pim_FricLrngRunOneTi() = TRUE;
	...
	for(Index_Cnt_T_u08 = 0U; Index_Cnt_T_u08 < VEHSPDPTNUM1_ULS_U08; Index_Cnt_T_u08++)
	{
		...
		Rte_Pim_FricLrngData()->VehLrndFric[Index_Cnt_T_u08] = Lim_f32(Rte_Pim_SatnAvrgFric()[Index_Cnt_T_u08], (Rte_Prm_SysFricLrngIgnCycFricChgLim_Val() - Rte_Pim_FricLrngData()->VehLrndFric[Index_Cnt_T_u08]), 
																	  (Rte_Prm_SysFricLrngIgnCycFricChgLim_Val() + Rte_Pim_FricLrngData()->VehLrndFric[Index_Cnt_T_u08]));
		
		Rte_Pim_FricLrngData()->VehLrndFric[Index_Cnt_T_u08] = Lim_f32(Rte_Pim_FricLrngData()->VehLrndFric[Index_Cnt_T_u08], VEHLRNDFRICLOLIM_ULS_F32, VEHLRNDFRICHILIM_ULS_F32);
	}	
	...
	if((*Rte_Pim_FricLrngOperModPrev() != Rte_Pim_FricLrngData()->SysFricLrngOperMod) && (*Rte_Pim_FricLrngRunOneTi() == TRUE))
	{
															   ...
															   P2VAR(boolean, AUTOMATIC, SysFricLrng_VAR) LrngEna_Cnt_T_Logl)
{
	...
	VAR(float32,AUTOMATIC) FilHwVel_HwRadPerSec_T_f32;
	...
	FilHwVel_HwRadPerSec_T_f32 = FilLpUpdOutp_f32((HwVel_HwRadPerSec_T_f32 * ARCHGLBPRM_180OVERPI_ULS_F32), Rte_Pim_HwVelLpFil()); 
	
	FilVehLatA_MtrPerSecSqd_T_f32 = FilLpUpdOutp_f32((VehLatA_MtrPerSecSqd_T_f32 * SCAGFACVEHLATA_MTRPERSECSQD_F32), Rte_Pim_LatALpFil());
	...
	HwVelConstraint(FilHwVel_HwRadPerSec_T_f32, &HwVelOK_Cnt_T_Logl, &HwVelDir_Cnt_T_u08);
															...
															
{
	...
	*Rte_Pim_PrevMaxRawAvrgFric()	= MaxRawAvrgFric_HwNwtMtr_T_f32;
	...
	{
		(void)Rte_Write_MaxLrndFric_Val(Lim_f32(MaxRawAvrgFric_HwNwtMtr_T_f32, MAXLRNDFRICLOLIM_HWNWTMTR_F32, MAXLRNDFRICHILIM_HWNWTMTR_F32));
	}
	else
	{
...
static FUNC(void, SysFricLrng_CODE) RangeCounterManager(VAR(uint16, AUTOMATIC) DeltaIdxOffs_Cnt_T_u16, VAR(uint16, AUTOMATIC) DeltaIdxOffsDec_Cnt_T_u16, VAR(uint16, AUTOMATIC) DeltaIdxOffsInc_Cnt_T_u16, VAR(uint16, AUTOMATIC) Gate_Cnt_T_u16)
{

	VAR(uint8, AUTOMATIC) Index_Cnt_T_u08;
	VAR(boolean, AUTOMATIC) RngCntrThdExcdd_Uls_T_Lgcl;

	
    /* Implementation of "Range Counter Manager" subsystem implementation*/    
    RngCntrThdExcdd_Uls_T_Lgcl = FALSE;
	
    for(Index_Cnt_T_u08 = 0U; (Index_Cnt_T_u08 < HWPTNUMSUB1_CNT_U08); Index_Cnt_T_u08++) 
    {
        if( Rte_Pim_FricLrngData()->RngCntr[DeltaIdxOffs_Cnt_T_u16][Index_Cnt_T_u08] >= Rte_Prm_SysFricLrngRngCntrThd_Val())
        {
            RngCntrThdExcdd_Uls_T_Lgcl = TRUE;
        }
    }
    ...
    if(TRUE == RngCntrThdExcdd_Uls_T_Lgcl)
    {
        for(Index_Cnt_T_u08 = 0U; Index_Cnt_T_u08 < HWPTNUMSUB1_CNT_U08; Index_Cnt_T_u08++ )
        {
...
static FUNC(void, SysFricLrng_CODE) RawAvrgCalc(VAR(uint8, AUTOMATIC) VehSpdIdx_Cnt_T_u08, 
												...
												VAR(boolean, AUTOMATIC) LrngEna_Cnt_T_Logl)
{ 
	VAR(float32, AUTOMATIC) RawAvg_HwNwtMtr_T_f32;
	...
	if(TRUE == LrngEna_Cnt_T_Logl)
	{
		(void)Rte_Write_SysFricEstimd_Val(Lim_f32(RawAvg_HwNwtMtr_T_f32, SYSFRICESTIMDLOLIM_HWNWTMTR_F32, SYSFRICESTIMDHILIM_HWNWTMTR_F32));
		(void)Rte_Write_SysSatnFricEstimd_Val(Lim_f32(SatnAvrgFric_HwNwtMtr_T_f32, SYSSATNFRICESTIMDLOLIM_HWNWTMTR_F32, SYSSATNFRICESTIMDHILIM_HWNWTMTR_F32));
	}
	else
	{
...
static FUNC(void, SysFricLrng_CODE) ResettingMode(P2VAR(float32, AUTOMATIC, SysFricLrng_VAR)FricOffs_HwNwtMtr_T_f32)
{
	...
	*FricOffs_HwNwtMtr_T_f32 = Lim_f32(((Rte_Pim_FricNonLrngData()->EolFric - Rte_Prm_SysFricLrngBasLineEolFric_Val()) * Rte_Prm_SysFricLrngEolFricDifScagFac_Val()), 
	...
	Rte_Pim_AvrgFricLpFil1()->FilSt = Fric_HwNwtMtr_f32;
	Rte_Pim_AvrgFricLpFil2()->FilSt = Fric_HwNwtMtr_f32;
	Rte_Pim_AvrgFricLpFil3()->FilSt = Fric_HwNwtMtr_f32;
	Rte_Pim_AvrgFricLpFil4()->FilSt = Fric_HwNwtMtr_f32;		
...
/*****************************************************************************************************************
  ...
  *			    
  * Inputs      	:   FilHwVel_HwRadPerSec_T_f32
  * Outputs    		:   HwVelOK_Cnt_T_Logl and HwVelDir_Cnt_T_u08
  * Returns    		:   None
  * Usage Notes 	:   None
  ****************************************************************************************************************/
static FUNC(void, SysFricLrng_CODE) HwVelConstraint(float32 FilHwVel_HwRadPerSec_T_f32, P2VAR(boolean, AUTOMATIC, SysFricLrng_VAR)HwVelOK_Cnt_T_Logl,
													P2VAR(uint8, AUTOMATIC, SysFricLrng_VAR)HwVelDir_Cnt_T_u08)
{
	VAR(uint8, AUTOMATIC) Index_Cnt_T_u08;
	VAR(float32, AUTOMATIC) HwVelBufSum_HwRadPerSec_T_f32;
	VAR(float32, AUTOMATIC) HwVelBufAvg_HwRadPerSec_T_f32;
	VAR(float32, AUTOMATIC) HwVelBufMin_HwRadPerSec_T_f32;
	VAR(float32, AUTOMATIC) HwVelBufMax_HwRadPerSec_T_f32;
	VAR(boolean, AUTOMATIC) VariNegRng_Cnt_T_Lgcl;
	VAR(boolean, AUTOMATIC) VariPosRng_Cnt_T_Lgcl;

	
	HwVelBufSum_HwRadPerSec_T_f32 = 0.0F;
	HwVelBufMin_HwRadPerSec_T_f32 = Rte_Pim_HwVelBuf()[0U];
	HwVelBufMax_HwRadPerSec_T_f32 = Rte_Pim_HwVelBuf()[0U];
	
	VariNegRng_Cnt_T_Lgcl = TRUE;
	VariPosRng_Cnt_T_Lgcl = TRUE;
	...
	for(Index_Cnt_T_u08 = 0U; Index_Cnt_T_u08 < IDXSELN1_ULS_U08; Index_Cnt_T_u08++)
	{
		...
		{
			VariNegRng_Cnt_T_Lgcl = FALSE; /* AllLTZ */
		}
		if(Rte_Pim_HwVelBuf()[Index_Cnt_T_u08] < 0.0F)
		{
			VariPosRng_Cnt_T_Lgcl = FALSE; /* AllGTZ */
		}
		/* Average */
		HwVelBufSum_HwRadPerSec_T_f32 += Rte_Pim_HwVelBuf()[Index_Cnt_T_u08];
		/* Range */
		HwVelBufMin_HwRadPerSec_T_f32 = Min_f32(HwVelBufMin_HwRadPerSec_T_f32, Rte_Pim_HwVelBuf()[Index_Cnt_T_u08]);
        HwVelBufMax_HwRadPerSec_T_f32 = Max_f32(HwVelBufMax_HwRadPerSec_T_f32, Rte_Pim_HwVelBuf()[Index_Cnt_T_u08]);
	}
	/* Range_Monotonicity Calculation  -  END*/
	
	/* Apply Criteria  - START */
	HwVelBufAvg_HwRadPerSec_T_f32 = Abslt_f32_f32(HwVelBufSum_HwRadPerSec_T_f32/(float32)IDXSELN1_ULS_U08);
	...
	if((HwVelBufAvg_HwRadPerSec_T_f32 <= Rte_Prm_SysFricLrngHwVelHiLim_Val())&&
		...
		((HwVelBufMax_HwRadPerSec_T_f32 - HwVelBufMin_HwRadPerSec_T_f32) <= Rte_Prm_SysFricLrngHwVelConstrLim_Val()) &&
		((VariNegRng_Cnt_T_Lgcl == TRUE) || (VariPosRng_Cnt_T_Lgcl == TRUE)))
	{
		*HwVelOK_Cnt_T_Logl = TRUE;
	}
	else
	...
	if(VariPosRng_Cnt_T_Lgcl == TRUE)
	{
		*HwVelDir_Cnt_T_u08 = HWVELINCDIR_ULS_U08;
	}
	else if(VariNegRng_Cnt_T_Lgcl == TRUE)
	{
	...
	Rte_Pim_HwVelBuf()[0] = FilHwVel_HwRadPerSec_T_f32;
