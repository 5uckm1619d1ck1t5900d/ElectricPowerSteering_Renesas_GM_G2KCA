...
class _Component():
    ...
    def generate_local_headers(self):
        ...
        if component_types_dir is not None:
            component_types_pattern = os.path.join(component_types_dir, '*{}.arxml'.format(self.short_name))
            component_files = [os.path.basename(file_name) for file_name in glob.glob(component_types_pattern)]
            if len(component_files) > 0:
                component_name = re.search(r'(.+)\.arxml', component_files[0]).group(1) or self.short_name
    ...
    def create_reports(self):
        ...
        if os.path.exists(self.bugfinder_path):
            # form command
            command = []
        ...
        if os.path.exists(self.codeprover_path):
            # form command
            command = []
            command.append('polyspace-report-generator')
            command.append('-template')
...
class _ComponentMenu(nxtr.Menu):
    ...
    def body(self, frame):
        ...
        # Generate section
        box = ttk.LabelFrame(frame, text='Generate', padding=int_pad, labelanchor='n')
        ttk.Button(box, width=b_width, text='Configurator', command=lambda: self.__exe_wait_dialog('generate_local_headers')).pack()
        box.pack(pady=sec_pad, fill='x')
